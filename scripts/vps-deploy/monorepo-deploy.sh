...
echo "=== W3 Suite Monorepo Deployment ==="
...
NODE_ENV=development npm i --legacy-peer-deps

echo ""
echo "Step 2: Build Backend API..."
npx esbuild apps/backend/api/src/index.ts --platform=node --packages=external --bundle --format=esm --outdir=dist

echo ""
echo "Step 3: Build W3 Frontend..."
cd apps/frontend/web
npx vite build
cd /var/www/w3suite

echo ""
echo "Step 4: Build Brand Frontend..."
...
pm2 list
echo ""
curl -s http://localhost:3004/api/health || echo "API starting..."
echo ""
