...
# Recent Changes
- **2025-11-16: Brand Interface Master Catalog System Complete** - Full implementation of Hybrid Minimal architecture for template governance across 300+ tenants. JSON file storage with Git versioning (automatic commit + push with author metadata) for campaigns/pipelines/funnels in `brand-templates/` directory. Database replication for workflows in `brand_workflows` table. Complete REST API (`/brand-api/templates/:type`, `/brand-api/workflows`) with cross-tenant support. TanStack Query integration via `useBrandTemplates.ts` and `useBrandWorkflows.ts` hooks. Fixed critical bugs: dynamic mutation selection per template type, proper workflow status handling including 'deprecated', parent component props alignment. Zero LSP errors, all integrations functional. System ready for batch deployment to tenants.
