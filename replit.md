# Overview
W3 Suite is an AI-powered, multi-tenant enterprise platform designed to centralize business operations across CRM, POS, WMS, Analytics, HR, and CMS modules. Its primary purpose is to enhance efficiency, provide data-driven insights, and support strategic decision-making through advanced AI capabilities. The project's long-term vision is to establish W3 Suite as a leading AI-native operating system in the market, delivering a comprehensive solution for modern enterprises.
...
# System Architecture
- **UI/UX Decisions**: The platform uses `shadcn/ui` (based on Radix UI) for consistent and accessible interfaces, adhering to a component-first approach. All pages maintain a standard app structure with headers, sidebars, and white backgrounds. Responsive scaling is achieved by exclusively using `rem` units for new UI components, controlled by `VITE_FONT_SCALE=80`. Forms incorporate Italian Business Validation with real-time feedback, localized error messages, and Zod schemas for data integrity. Design tokens and templates are preferred over inline styles and custom layouts.
