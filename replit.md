...
- **Technical Implementations**:
    ...
    - **MCP Public Gateway (External API)**: Provides a JSON-RPC 2.0 interface (`POST /api/mcp-public/sse`) for external integrations. **Hybrid Authentication**: API Key (`sk_live_*`) for n8n/Zapier/scripts OR OAuth2 JWT (`eyJ*`) for ChatGPT/Claude Desktop. OAuth2 clients: `chatgpt-mcp-client`, `claude-mcp-client`, `n8n-mcp-client`, `zapier-mcp-client`. MCP scopes: `mcp_read`, `mcp_write`. Full docs: `docs/MCP-PUBLIC-GATEWAY-API.md`.
