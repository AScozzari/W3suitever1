# Overview
W3 Suite is an AI-powered, multi-tenant enterprise platform centralizing business operations across CRM, POS, WMS, Analytics, HR, and CMS modules. Its primary goal is to enhance efficiency, provide data-driven insights, and support strategic decision-making through advanced AI. The project aims to position W3 Suite as a leading AI-native operating system in the market.
...
# System Architecture
- **UI/UX Decisions**: The platform utilizes `shadcn/ui` (built on Radix UI) for a consistent, accessible user interface, featuring standardized headers, sidebars, white backgrounds, and Tailwind CSS. All new UI development must exclusively use `rem` units to ensure responsive scaling, controlled by `VITE_FONT_SCALE=80`. Forms incorporate Italian Business Validation with real-time feedback, localized error messages, and Zod schemas.
- **Technical Implementations**: The backend is built on PostgreSQL, employing a 3-schema architecture (`w3suite`, `public`, `brand_interface`). Security features include OAuth2/OIDC, MFA, JWTs, and a 3-level RBAC with Row-Level Security (RLS) managed by `app.tenant_id`. Key modules comprise a Universal Workflow Engine, Unified Notification System, Centralized Webhook Management, Task Management, and Multi-Provider OAuth (MCP). AI capabilities include an AI Voice Agent with RAG, AI Enforcement Middleware, an AI Workflow Builder, and Intelligent Workflow Routing. The WMS module supports diverse product types with dual-layer versioning, 13 logistic states, serialized/non-serialized items, immutable event logs, and historical snapshots. The Brand Interface provides a Workflow Builder and a Git-versioned JSON-based Master Catalog System. The Commissioning Module calculates commissions across three architectural levels (Type, Template, Instance) using configurable functions and variables. GTM integration leverages `tenant_gtm_config` with Mixed RLS and `store_tracking_config` for store-specific tracking IDs, with API Secrets encrypted via `EncryptionKeyService`.
- **System Design Choices**: The platform's hierarchical structure is Tenant → Commercial Area → Organization Entity → Store → Department → Team → User. A Cross-Store Architecture provides tenant-wide data visibility with Role-Based Access (RBA) and advanced filtering. Request routing adheres to "Functional First → First Wins" and "Shift-Based Routing." All system actions are managed via `action_definitions`, which serve as the single source of truth for the MCP Gateway, with routing handled by `UnifiedTriggerService`. User scope data is derived from `user_stores`. VPS deployment is incremental, utilizing `deploy/incremental-deploy.sh` scripts to `/var/www/w3suite/`, secured via SSH with `deploy/keys/vps_key`. Frontend builds require `VITE_AUTH_MODE=oauth2` and `VITE_FONT_SCALE=80`. The `w3suite_prod` database is accessed exclusively via a local socket over SSH.
