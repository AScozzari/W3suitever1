...
# Recent Changes
- **2025-11-16: Brand Interface Workflow Builder with Drag-and-Drop** - Completato Workflow Builder canvas utilizzando ReactFlow con drag-and-drop completo dalla palette alla canvas. Importati 29+ workflow node types direttamente da W3 Suite (`workflow-node-definitions.ts`) organizzati in 5 categorie (Triggers, Actions, AI, Routing, Flow Control). Implementato onDragStart sui NodePaletteItem, onDrop/onDragOver sulla canvas ReactFlow, conversione coordinate con screenToFlowPosition. Fix critical scope bug: spostato Import Templates Dialog da TemplatesTab (componente figlio) a BrandCRM (componente padre) risolvendo errore "showImportDialog is not defined". Architect-approved, zero LSP errors. Componente BrandWorkflowsTab (733 linee) con node palette collapsible, ReactFlow canvas, 29 executor types, attributi draggable su tutti i nodi.
