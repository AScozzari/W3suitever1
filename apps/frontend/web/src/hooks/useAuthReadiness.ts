    ...
    const waitForAuth = async () => {
      ...
      const checkReady = async () => {
        try {
          ...
          // Verifica OAuth2 token disponibile in production mode
          let tokenValid = true;
          if (authMode === 'oauth2') {
            try {
          ...
          return tenantValid && localStorageValid && authModeValid && tokenValid;
        } catch (error) {
      ...
      while (!(await checkReady()) && currentAttempts < maxAttempts) {
