...
export function useSIPRegistration(): UseSIPRegistrationReturn {
  ...
  // Fetch SIP credentials from backend - wait for auth to be ready
  ...
  const { data: credentials, isLoading, error } = useQuery<SIPCredentials>({
    ...
    enabled: isAuthReady, // Wait for auth initialization before fetching
  ...
  useEffect(() => {
    ...
    const setupSIP = async () => {
      try {
        ...
        // Use sipServer for WebSocket host (not authRealm which is for SIP digest auth)
        const wsHost = credentials.sipServer;
        const wsPort = credentials.wsPort || 443;
        // Respect empty wsPath - don't force '/ws' if not configured
        const wsPath = credentials.wsPath ?? '';
        const transport = credentials.transport?.toLowerCase() || 'wss';
        const protocol = transport === 'ws' ? 'ws' : 'wss';
        // Don't include port if it's the default (443 for wss, 80 for ws)
        const isDefaultPort = (protocol === 'wss' && wsPort === 443) || (protocol === 'ws' && wsPort === 80);
        const wsServer = isDefaultPort 
          ? `${protocol}://${wsHost}${wsPath}`
