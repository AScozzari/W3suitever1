    ...
    this.businessRules.push({
      ...
      validate: (nodes: Node[], edges: Edge[]) => {
        ...
        const hrActions = nodes.filter(n => {
          const actionId = typeof n.data?.actionId === 'string' ? n.data.actionId : '';
          return n.data?.category === 'hr' || actionId.includes('hr');
        });
    ...
    this.businessRules.push({
      ...
      validate: (nodes: Node[], edges: Edge[]) => {
        ...
        const expenseNodes = nodes.filter(n => {
          const actionId = typeof n.data?.actionId === 'string' ? n.data.actionId : '';
          return actionId.includes('expense') || n.data?.category === 'finance';
        });
