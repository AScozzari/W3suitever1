...
export class WorkflowExecutionEngine {
  ...
  private async executeActionNode(node: Node, context: ExecutionContext): Promise<ExecutionResult> {
    ...
    try {
      ...
      const actionId = typeof actionData.actionId === 'string' ? actionData.actionId : '';
      if (actionId === 'hr-approval' || actionId === 'expense-approval') {
        const approverRole = typeof actionData.approverRole === 'string' ? actionData.approverRole : 'manager';
        const actionLabel = typeof actionData.label === 'string' ? actionData.label : 'action';
        const message = typeof actionData.message === 'string' ? actionData.message : `Please approve: ${actionLabel}`;
  ...
  private async executeApprovalNode(node: Node, context: ExecutionContext): Promise<ExecutionResult> {
    ...
    try {
      ...
      const approverRole = typeof approvalData.approverRole === 'string' ? approvalData.approverRole : 'manager';
      const message = typeof approvalData.message === 'string' ? approvalData.message : 'Please review and approve';
