  ...
  const kpiCards: KPIMetric[] = [
    {
      ...
      change: metrics?.attendanceRate ? (metrics.attendanceRate ?? 0) - 95 : 0,
      changeType: (metrics?.attendanceRate ?? 0) > 95 ? 'increase' : 'decrease',
      icon: UserCheck,
      color: "text-green-600 dark:text-green-400",
      sparklineData: metrics?.trends?.attendance?.slice(-7)?.map(d => ({ value: d.value })) || [],
      target: 95,
      status: (metrics?.attendanceRate ?? 0) >= 95 ? 'success' : (metrics?.attendanceRate ?? 0) >= 90 ? 'warning' : 'critical',
    },
    {
      ...
      unit: 'ore',
      status: (metrics?.overtimeHours ?? 0) > 200 ? 'warning' : 'neutral',
    },
    {
    ...
    },
    {
      ...
      change: metrics?.absenteeismRate ? (metrics.absenteeismRate ?? 0) - 3 : 0,
      changeType: (metrics?.absenteeismRate ?? 0) > 3 ? 'increase' : 'decrease',
      icon: AlertTriangle,
      color: "text-red-600 dark:text-red-400",
      target: 3,
      status: (metrics?.absenteeismRate ?? 0) <= 3 ? 'success' : (metrics?.absenteeismRate ?? 0) <= 5 ? 'warning' : 'critical',
    },
    {
      title: "Turnover Rate",
      ...
      target: 10,
      status: (metrics?.turnoverRate ?? 0) <= 10 ? 'success' : (metrics?.turnoverRate ?? 0) <= 15 ? 'warning' : 'critical',
    },
    {
    ...
    },
    {
      ...
      change: (metrics?.pendingApprovals ?? 0) > 5 ? 100 : 0,
      changeType: (metrics?.pendingApprovals ?? 0) > 5 ? 'increase' : 'decrease',
      icon: Calendar,
      color: "text-yellow-600 dark:text-yellow-400",
      status: (metrics?.pendingApprovals ?? 0) === 0 ? 'success' : (metrics?.pendingApprovals ?? 0) <= 5 ? 'warning' : 'critical',
    },
    {
      title: "Compliance Score",
      value: `${metrics?.complianceScore?.toFixed(0) || 0}%`,
      change: metrics?.complianceScore ? (metrics.complianceScore ?? 0) - 90 : 0,
      changeType: (metrics?.complianceScore ?? 0) >= 90 ? 'increase' : 'decrease',
      icon: CheckCircle,
      color: "text-teal-600 dark:text-teal-400",
      target: 90,
      status: (metrics?.complianceScore ?? 0) >= 90 ? 'success' : (metrics?.complianceScore ?? 0) >= 80 ? 'warning' : 'critical',
    },
  ];
