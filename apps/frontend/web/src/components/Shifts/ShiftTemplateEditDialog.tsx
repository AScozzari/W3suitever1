...
function getDefaultFormValues(template: ShiftTemplate | null): ShiftTemplateFormData {
  ...
  return {
    ...
    status: (template.status as 'active' | 'archived') || 'active',
...
export default function ShiftTemplateEditDialog({ isOpen, onClose, template }: Props) {
  ...
  const { data: coverage, isLoading: coverageLoading } = useQuery({
    ...
    enabled: !!template?.id && !!selectedStoreId && isOpen
  });

  const editMutation = useMutation({
    mutationFn: async (data: ShiftTemplateFormData) => {
      if (!template?.id) throw new Error('Template ID mancante');
      ...
      return await apiRequest(`/api/hr/shift-templates/${template.id}`, { 
        method: 'PUT', 
    ...
    onSuccess: () => {
      ...
      toast({ 
        ...
        description: "È stata creata una nuova versione del template. I turni futuri sono stati aggiornati." 
      });
      handleClose();
    },
    onError: (error: any) => {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField control={form.control} name="name" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input {...field} placeholder="es. Turno Mattina Standard" data-testid="input-edit-template-name" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                ...
                <FormField control={form.control} name="description" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea {...field} placeholder="Descrizione opzionale del template..." className="min-h-[80px]" data-testid="input-edit-template-description" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                  ...
                  <FormField control={form.control} name="storeId" render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} value={field.value} disabled={storesLoading}>
                        <FormControl><SelectTrigger data-testid="select-edit-template-store"><SelectValue placeholder="Seleziona punto vendita" /></SelectTrigger></FormControl>
                        <SelectContent>
                        ...
                        <div className="mt-2">
                          {coverage.sufficient ? (
                            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                              <CheckCircle className="w-3 h-3 mr-1" />{coverage.available} risorse disponibili
                            </Badge>
                          ) : (
                            <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200">
                              <AlertTriangle className="w-3 h-3 mr-1" />Solo {coverage.available} risorse
                            </Badge>
                          )}
                        </div>
                      )}
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <RadioGroup onValueChange={field.onChange} value={field.value} className="flex gap-4 pt-2">
                          <div className="flex items-center space-x-2"><RadioGroupItem value="active" id="edit-status-active" /><Label htmlFor="edit-status-active" className="cursor-pointer">Attivo</Label></div>
                          <div className="flex items-center space-x-2"><RadioGroupItem value="archived" id="edit-status-archived" /><Label htmlFor="edit-status-archived" className="cursor-pointer">Archiviato</Label></div>
                        </RadioGroup>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                      ...
                      <FormControl>
                        <RadioGroup onValueChange={field.onChange} value={field.value} className="flex flex-col gap-3 pt-2">
                          <div className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                            <RadioGroupItem value="slot_based" id="edit-shift-type-slot" className="mt-1" />
                            <div className="flex-1">
                              <Label htmlFor="edit-shift-type-slot" className="cursor-pointer font-medium">Turnazione a Fascia</Label>
                              <p className="text-sm text-muted-foreground mt-1">Ogni fascia oraria è un turno separato</p>
                            </div>
                          </div>
                          <div className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                            <RadioGroupItem value="split_shift" id="edit-shift-type-split" className="mt-1" />
                            <div className="flex-1">
                              <Label htmlFor="edit-shift-type-split" className="cursor-pointer font-medium">Turnazione Spezzata</Label>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={addTimeSlot} disabled={fields.length >= 5} data-testid="button-edit-add-time-slot"><Plus className="w-4 h-4 mr-2" />Aggiungi</Button>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div><span className="text-sm text-muted-foreground">Ore Totali Template</span><div className="text-2xl font-bold text-blue-700">{totalHours.toFixed(1)}h</div></div>
                  </div>
                  <Badge variant="outline" className="bg-blue-100 text-blue-700 border-blue-200">{fields.length} {fields.length === 1 ? 'fascia' : 'fasce'}</Badge>
                </div>
              </CardContent>
            </Card>
            ...
            <DialogFooter className="gap-2">
              ...
              <Button type="submit" disabled={editMutation.isPending} className="bg-gradient-to-r from-orange-500 to-orange-600" data-testid="button-save-edit-template">
                {editMutation.isPending ? <><Save className="w-4 h-4 mr-2 animate-spin" />Salvando...</> : <><Save className="w-4 h-4 mr-2" />Aggiorna Template</>}
              </Button>
            </DialogFooter>
          </form>
        </Form>
