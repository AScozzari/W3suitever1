...
import { timeSlotSchema, shiftTemplateSchema, TEMPLATE_COLORS, type ShiftTemplateFormData } from './shiftTemplateSchemas';
...
export default function ShiftTemplateCreateDialog({ isOpen, onClose }: Props) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Template Creato", description: "Il nuovo template turni è stato creato con successo" });
      handleClose();
    },
    onError: (error: any) => {
      toast({
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter className="flex justify-between">
          <Button variant="outline" onClick={handleClose} disabled={createMutation.isPending}>Annulla</Button>
          <Button onClick={form.handleSubmit(handleSubmit)} disabled={createMutation.isPending} className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700" data-testid="button-save-template">
            {createMutation.isPending ? (<><div className="w-4 h-4 mr-2 border-2 border-white border-t-transparent rounded-full animate-spin" />Salvando...</>) : (<><Save className="w-4 h-4 mr-2" />Crea Template</>)}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
