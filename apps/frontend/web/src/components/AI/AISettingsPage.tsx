...
export default function AISettingsPage() {
  ...
  // Fetch AI settings
  // NOTE: queryClient auto-extracts .data from API response, so settings IS the AISettings object directly
  const { data: settings, isLoading: settingsLoading, error: settingsError } = useQuery<AISettings>({
  ...
  useEffect(() => {
    console.log('[AI-SETTINGS] 🔍 Settings changed:', {
      ...
      error: settingsError,
      hasData: !!settings,
      dataReceived: settings
    });
    
    // NOTE: queryClient auto-extracts .data, so settings IS the AISettings object directly
    if (settings) {
      ...
      const newFormData = {
        ...
        apiConnectionStatus: settings.apiConnectionStatus || 'disconnected',
        // 🔑 CRITICAL: Preserve user-entered API key, don't overwrite with masked key
        openaiApiKey: userEnteredApiKey !== null ? userEnteredApiKey : settings.openaiApiKey
      };
