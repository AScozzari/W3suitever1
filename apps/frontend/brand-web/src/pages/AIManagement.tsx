...
export default function AIManagement() {
  ...
  const { data: agentsData, isLoading: agentsLoading } = useQuery({
    ...
    queryFn: async () => {
      return apiRequest(`/brand-api/ai/agents?${filterParams}`);
    },
  ...
  const { data: configurationsData, isLoading: configurationsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return apiRequest(`/brand-api/ai/configurations?${params.toString()}`);
    },
      ...
      <div style={{ padding: '0 24px 24px' }}>
        ...
        {showKnowledgeModal && knowledgeAgent && (
          <AgentDetailsModal
            ...
            agentId={knowledgeAgent.agentId}
            agentName={knowledgeAgent.name}
            agentConfig={{
              systemPrompt: knowledgeAgent.systemPrompt,
              description: knowledgeAgent.description,
              role: knowledgeAgent.role,
              temperature: knowledgeAgent.temperature,
              llmModel: knowledgeAgent.llmModel
            }}
          />
        )}
      </div>
