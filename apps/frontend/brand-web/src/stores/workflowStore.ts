// 🏪 BRAND INTERFACE WORKFLOW STATE MANAGEMENT
// Zustand store for local workflow editor state (no persistence)
// Templates are managed via TanStack Query + API
...
interface WorkflowState {
  ...
  selectedCategory: string | null; // For category filtering
...
export const useWorkflowStore = create<WorkflowState & WorkflowActions>()(
  subscribeWithSelector(
    immer((set, get) => ({
        ...
        // 🎯 NODE OPERATIONS
        addNode: (node: Node) => {
          console.log('🏪 BRAND STORE: Adding node:', node);
          set((state) => {
