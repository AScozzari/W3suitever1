...
export class HttpStreamingManager {
  ...
  async createSession(params: {
    ...
    callerNumber: string;
    aiAgentRef: string;
  }): Promise<{ sessionId: string; status: string; error?: string; reason?: string }> {
    ...
    // 🔒 CHECK TENANT AI STATUS BEFORE CONNECTING TO OPENAI
    const aiStatus = await this.checkTenantAIStatus(tenantId);
    if (!aiStatus.enabled) {
      logger.info('[HTTP Streaming] AI disabled for tenant', {
        callId,
        tenantId,
    ...
    // Create OpenAI client with Voice Gateway's API key
    // (tenant-specific keys are used by the backend for chat/document analysis)
  ...
  /**
   * Check if AI is enabled for the tenant
   * Returns enabled/disabled status with reason
   * 
   * IMPORTANT: The /api/ai/status endpoint ALWAYS returns HTTP 200 with success:true
   * We must check the `data.enabled` field to determine if AI is actually enabled
   */
  private async checkTenantAIStatus(tenantId: string): Promise<{
    enabled: boolean;
    reason: string;
    message: string;
    apiKey?: string;
    ...
    try {
      const response = await fetch(`${this.w3ApiUrl}/api/ai/status`, {
        headers: {
          'x-tenant-id': tenantId,
          'x-api-key': this.w3ApiKey,
          'X-Auth-Session': 'authenticated',
          'X-Demo-User': 'voice-gateway'
        }
      });

      // Parse JSON response - even if HTTP status is 200, AI might be disabled
      const jsonData = await response.json();
      
      // Extract the actual status from the response
      // The endpoint returns { success: true, data: { enabled, reason, message, ... } }
      const status = jsonData.data || jsonData;

      // 🔒 CRITICAL: Check if AI is actually enabled (not just HTTP 200)
      if (!status.enabled) {
        logger.info('[HTTP Streaming] AI is disabled for tenant', {
          tenantId,
        ...
        return {
          enabled: false,
          reason: status.reason || 'disabled',
          message: status.message || 'AI disabilitato per questo tenant'
        };
      }

      // AI is enabled - Voice Gateway will use its own OPENAI_API_KEY
      // (tenant API keys are used by the backend directly, not by voice gateway)
      logger.info('[HTTP Streaming] AI is enabled for tenant', {
        tenantId,
        reason: status.reason,
        hasApiKey: status.hasApiKey
      });
      ...
      return {
        enabled: true,
        reason: status.reason || 'active',
        message: status.message || 'AI attivo'
      };

    } catch (error: any) {
      logger.error('[HTTP Streaming] Error checking AI status', {
        tenantId,
        error: error.message
      });
      ...
      return {
        ...
        message: 'Errore nella connessione al servizio AI'
      };
    }
  }
