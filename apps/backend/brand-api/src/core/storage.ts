...
import { eq, and, sql, inArray, like, or, count, desc, isNotNull } from "drizzle-orm";
import type { 
...
class BrandDrizzleStorage implements IBrandStorage {
  ...
  async syncBranchesFromTenants(): Promise<{ created: number; updated: number; total: number }> {
    try {
      console.log('🔄 [SYNC-BRANCHES] Starting branch synchronization from W3Suite tenants...');
      ...
      // Get all active tenants from W3Suite database (must have non-null slug)
      const tenants = await w3db.select({
        id: w3Tenants.id,
        name: w3Tenants.name,
        slug: w3Tenants.slug,
        status: w3Tenants.status
      })
      .from(w3Tenants)
      .where(and(
        eq(w3Tenants.status, 'active'),
        isNotNull(w3Tenants.slug)
      ));
      
      console.log(`📊 [SYNC-BRANCHES] Found ${tenants.length} active tenants in W3Suite`);
      
      for (const tenant of tenants) {
        // Create/update main branch for tenant: {tenant-slug}
        ...
        const stores = await w3db.select({
          ...
          code: w3Stores.code,
          nome: w3Stores.nome,
          status: w3Stores.status
        })
        .from(w3Stores)
        .where(and(
          eq(w3Stores.tenantId, tenant.id),
          eq(w3Stores.status, 'active')
        ));
        
        console.log(`📍 [SYNC-BRANCHES] Found ${stores.length} active stores for tenant ${tenant.slug}`);
        
        // Create/update branch for each store: {tenant-slug}/{store-code}
      ...
      console.log(`🎉 [SYNC-BRANCHES] Synchronization complete! Created: ${created}, Updated: ${updated}, Total: ${total}`);
