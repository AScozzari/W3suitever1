  ...
  static async getValidCredentials(params: CredentialLookupParams): Promise<CredentialPayload> {
    ...
    try {
      ...
      const decryptedCredentials = await decryptMCPCredentials(
        cred.encryptedCredentials as Record<string, any>,
        tenantId
      );
      ...
      return {
        credentialId: cred.id,
        credentialType: cred.credentialType as CredentialPayload['credentialType'],
  ...
  static async getValidCredentialsWithFallback(
    ...
    try {
      ...
      if (allowTenantFallback) {
        ...
        if (tenantCred) {
          const decryptedCredentials = await decryptMCPCredentials(
            tenantCred.encryptedCredentials as Record<string, any>,
            tenantId
          );
          ...
          return {
            credentialId: tenantCred.id,
            credentialType: tenantCred.credentialType as CredentialPayload['credentialType'],
            credentials: decryptedCredentials,
