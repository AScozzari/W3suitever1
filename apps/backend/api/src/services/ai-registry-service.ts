...
import { aiAgentsRegistry, AIAgentRegistry } from '../db/schema/brand-interface';
import { db } from '../core/db';
import { eq } from 'drizzle-orm';

// AgentProfile type for runtime usage
...
export class AIRegistryService {
  ...
  private async processWithRegistryAgent(
    ...
    const enhancedSettings = {
      ...settings,
      systemPrompt: settingsAny.systemPrompt ?? this.buildAgentSystemPrompt(agent)
    };
  ...
  private async loadAgentRegistry(): Promise<void> {
    try {
      ...
      // Using main db connection which has access to brand_interface schema
