...
export class MetaOAuthService {
   ...
   * Removes permissions for the app from the user's account
   */
  static async revokeAccess(accessToken: string): Promise<void> {
    try {
      const revokeUrl = `${this.GRAPH_API_BASE}/${this.GRAPH_API_VERSION}/me/permissions?access_token=${accessToken}`;
      
      const response = await fetch(revokeUrl, { method: 'DELETE' });
      
      if (!response.ok) {
        const error = await response.text();
        logger.warn('⚠️ [Meta OAuth] Token revocation returned non-OK status', {
          status: response.status,
          error
        });
      } else {
        logger.info('✅ [Meta OAuth] Token revoked successfully');
      }
    } catch (error) {
      logger.error('❌ [Meta OAuth] Token revocation failed', {
  ...
  static async handleCallback(params: {
    ...
    try {
      ...
      const encryptedCreds = await encryptMCPCredentials({
      ...
      for (const page of pages) {
        ...
        const encryptedPageToken = await encryptMCPCredentials({
