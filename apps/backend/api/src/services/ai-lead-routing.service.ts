...
export class AILeadRoutingService {
  ...
  async routeLead(input: LeadRoutingInput): Promise<LeadRoutingOutput> {
    ...
    try {
      ...
      // Note: aiAgentsRegistry is in brand_interface schema (no RLS), but we scope to status=active
      ...
      const agent = agentConfig[0] as any;
      ...
      const systemPrompt = (agent?.promptTemplate || agent?.systemPrompt) || this.getDefaultSystemPrompt();
