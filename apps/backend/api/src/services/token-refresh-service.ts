...
export class TokenRefreshService {
  ...
  private static async refreshCredential(credential: any): Promise<void> {
    ...
    switch (oauthProvider) {
      ...
      case 'meta':
        // Meta tokens are long-lived (60 days) and can't be refreshed the same way
        // They need to be re-exchanged before expiry using getLongLivedToken
        refreshedCreds = decryptedCreds; // Keep current credentials
        break;
