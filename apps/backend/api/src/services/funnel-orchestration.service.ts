...
export class FunnelOrchestrationService {
  ...
  async exitFunnel(params: {
    ...
    try {
      ...
      // Update deal - use wonAt or lostAt based on exit reason
      const [updatedDeal] = await db
        .update(crmDeals)
        .set({
          status: newStatus,
          wonAt: params.exitReason === 'won' ? new Date() : null,
          lostAt: params.exitReason === 'lost' ? new Date() : null,
          updatedAt: new Date()
        } as any)
        .where(
          and(
      ...
      logger.info('✅ [FUNNEL-ORCHESTRATION] Funnel exit completed', {
        ...
        exitReason: params.exitReason,
        status: newStatus,
        value: updatedDeal.estimatedValue
      });
      ...
      return {
        ...
        data: {
          ...
          status: newStatus,
          value: updatedDeal.estimatedValue,
          closedAt: params.exitReason === 'won' ? updatedDeal.wonAt : updatedDeal.lostAt,
