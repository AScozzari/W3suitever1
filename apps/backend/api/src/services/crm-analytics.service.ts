...
class CRMAnalyticsService {
  ...
  async getExecutiveSummary(filters: AnalyticsFilters): Promise<ExecutiveSummary> {
    ...
    const deals = dealResult.rows[0] as any;
    const customers = customerResult.rows[0] as any;
    
    const conversionRate = (metrics?.total_leads || 0) > 0 ? 
      ((metrics?.converted_leads || 0) / (metrics?.total_leads || 1)) * 100 : 0;
