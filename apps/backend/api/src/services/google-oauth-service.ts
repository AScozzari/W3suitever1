...
export class GoogleOAuthService {
  ...
  static async refreshAccessToken(params: {
    ...
    try {
      ...
      const oauth2Client = await this.getOAuth2Client('', tenantId); // No redirect needed for refresh
  ...
  private static async updateStoredCredentials(params: {
    ...
    try {
      ...
      const encryptResult = await encryptMCPCredentials(
        credentials,  // First: data to encrypt
        tenantId      // Second: tenant ID for key derivation
      );
      const encryptedData = encryptResult.encrypted;
      const keyId = encryptResult.keyId;
   ...
   */
  static async revokeAccess(accessToken: string, tenantId: string = 'system'): Promise<void> {
    try {
