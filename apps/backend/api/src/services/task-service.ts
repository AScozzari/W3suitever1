  ...
  static async createTaskComplete(data: {
    ...
    return await db.transaction(async (tx) => {
      ...
      if (assignmentsToInsert.length > 0) {
        await tx.insert(taskAssignments).values(assignmentsToInsert as any);
      }
      ...
      if (data.checklistItems && data.checklistItems.length > 0) {
        ...
        await tx.insert(taskChecklistItems).values(checklistToInsert as any);
      }
  ...
  static async duplicateTask(taskId: string, tenantId: string, userId: string): Promise<Task> {
    return await db.transaction(async (tx) => {
      ...
      if (originalAssignments.length > 0) {
        ...
        await tx.insert(taskAssignments).values(newAssignments as any);
      }
      ...
      if (originalChecklist.length > 0) {
        ...
        await tx.insert(taskChecklistItems).values(newChecklist as any);
      }
