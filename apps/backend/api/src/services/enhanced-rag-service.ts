...
export class EnhancedRAGService {
  ...
  constructor() {
    // Lazy initialization - only create OpenAI client if API key is available
    if (process.env.OPENAI_API_KEY) {
      this.openaiClient = new OpenAI({
        apiKey: process.env.OPENAI_API_KEY
      });
    } else {
      console.warn('[ENHANCED-RAG] ⚠️ OPENAI_API_KEY not set - RAG service will be disabled');
    }
  }
  ...
  private async searchOverrideEmbeddings(
    ...
    try {
      ...
      for (const embeddingRow of embeddings) {
        try {
          // embedding is stored as vector type, convert to array
          const embeddingArray = Array.isArray(embeddingRow.embedding) ? embeddingRow.embedding : [];
          const similarity = this.calculateCosineSimilarity(queryEmbedding, embeddingArray as number[]);
          
          results.push({
            id: embeddingRow.id,
            agentId,
            sourceType: 'tenant_override',
            contentChunk: embeddingRow.contentChunk || '',
            similarity,
            source: {
              sourceUrl: embeddingRow.sourceUrl || undefined,
              origin: 'tenant',
              tenantId: embeddingRow.tenantId
            },
            metadata: embeddingRow.metadata || {}
          });
        } catch (parseError) {
          console.warn(`[ENHANCED-RAG] ⚠️ Failed to parse embedding ${embeddingRow.id}:`, parseError);
        }
      }
    ...
    try {
      ...
      for (const embeddingRow of embeddings) {
        try {
          ...
          console.warn(`[ENHANCED-RAG] ⚠️ Failed to parse cross-tenant embedding ${embeddingRow.id}:`, parseError);
        }
      }
  ...
  async getKnowledgeStats(agentId: string, tenantId: string): Promise<{
    ...
    try {
      ...
      // Count cross-tenant embeddings (table may not exist in all deployments)
      let crossTenantCount = { count: 0 };
      try {
        const result = await db.execute(sql`
          SELECT COUNT(*)::int as count FROM w3suite.ai_cross_tenant_embeddings 
          WHERE agent_id = ${agentId}
        `);
        crossTenantCount = { count: Number((result.rows[0] as any)?.count || 0) };
      } catch (e) {
        // Table may not exist
      }
