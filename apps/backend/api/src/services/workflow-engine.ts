...
// ReactFlow node/edge types for workflow parsing
...
export interface WorkflowExecutionContext {
  ...
  // 🌉 ReactFlow workflow execution context
  templateId?: string;
...
export class WorkflowEngine {
  ...
  async createInstanceFromReactFlow(
    ...
    try {
      ...
      const parsedWorkflow = await reactFlowBridgeParser.parseWorkflow(reactFlowData, {
        ...
        department: (template as any).for_department || undefined
      });
      ...
      const [instance] = await db
        ...
        .values({
          ...
          workflowData: {
            ...
            currentStepIndex: 1,
            parsedWorkflow: serializedWorkflow,
    ...
    try {
      ...
      if (approverLogic.teamId) {
        ...
        if (team) {
          ...
          if (team.primarySupervisorUser === approverId) {
            return true;
          }

          // Check if user is secondary supervisor
          if (team.secondarySupervisorUser === approverId) {
            return true;
          }
  ...
  async executeReactFlowWorkflow(
    ...
    try {
      ...
      // Parse ReactFlow workflow - cast unknown types to ReactFlow types
      const templateNodes = template.nodes as ReactFlowNode[] || [];
      const templateEdges = template.edges as ReactFlowEdge[] || [];
      const templateViewport = template.viewport as { x: number; y: number; zoom: number } || { x: 0, y: 0, zoom: 1 };
      ...
      while (currentStep) {
        ...
        // Note: ParsedWorkflowStep uses config.name for step label
        const stepName = (currentStep.config as any)?.name || currentStep.executorId || currentStep.nodeId;
        
        if (currentStep.type === 'approval') {
          logger.info('⏸️  [ENGINE] Pausing at approval step', {
          ...
          return {
            ...
            message: `Workflow paused at ${stepName} (requires manual action)`
          };
        }
        ...
        if (!result.success) {
          ...
          return {
            success: false,
            finalStatus: 'failed',
            message: `Failed at step: ${stepName}`
          };
        }
        ...
        currentStep = nextStep !== undefined ? nextStep : null as any;
      }
  ...
  private async findTeamAssignee(
    ...
    if (teamAssignments.length > 0) {
      ...
      return assignment.primarySupervisorUser || context.requesterId;
    }
