...
export class EmbeddingPipelineService {
  ...
  constructor() {
    ...
    if (process.env.OPENAI_API_KEY) {
      ...
      console.warn('[EMBEDDING-PIPELINE] ⚠️ OPENAI_API_KEY not set - embedding service will be disabled');
    }
  }
  ...
  async removeEmbeddings(agentId: string, tenantId?: string, sourceId?: string): Promise<number> {
    try {
      ...
      if (tenantId) {
        ...
        if (sourceId) {
          // Rimuovi source specifico - using 'archived' status
          ...
          // Rimuovi tutto per agente - using 'archived' status
          const result = await db.update(vectorEmbeddings)
            .set({ status: 'archived', updatedAt: new Date() })
        ...
        // Rimuovi da cross-tenant table using vectorEmbeddings with origin='brand'
        if (sourceId) {
          const result = await db.update(vectorEmbeddings)
            .set({ status: 'archived', updatedAt: new Date() })
            .where(
              and(
                ...
                eq(vectorEmbeddings.sourceId, sourceId),
