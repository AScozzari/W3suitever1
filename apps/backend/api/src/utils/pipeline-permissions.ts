...
export async function canUserPerformAction(
  ...
  try {
    // 1. Fetch pipeline settings (with tenant isolation via join with crmPipelines)
    const results = await db
      .select({
        settings: crmPipelineSettings,
        pipeline: crmPipelines
      })
      .from(crmPipelineSettings)
      .innerJoin(crmPipelines, eq(crmPipelines.id, crmPipelineSettings.pipelineId))
      .where(
        and(
  ...
  const supervisorTeams = await db
    ...
    .where(
      and(
        ...
        eq(teams.primarySupervisorUser, userId)
      )
    );
