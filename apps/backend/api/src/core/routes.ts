...
const universalRequestFiltersSchema = z.object({
  ...
  // Pagination & sorting - use coerce for proper type inference
...
// HR Request creation schema
const createHrRequestBodySchema = z.object({
...
// HR Request filters schema for listing - matches HRRequestListOptions interface
const hrRequestFiltersSchema = z.object({
  ...
  page: z.coerce.number().int().min(1).default(1),
  limit: z.coerce.number().int().min(1).max(100).default(20),
  sortBy: z.enum(['created', 'updated', 'priority', 'startDate']).default('created'),
  sortOrder: z.enum(['asc', 'desc']).default('desc')
});

// Type for HR Request insert data (used by storage)
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const devAuthMiddleware = (req: any, res: any, next: any) => {
    ...
    // OAuth2 mode: validate JWT token
    if (config.AUTH_MODE === 'oauth2') {
      const fullPath = req.originalUrl || req.url || req.path;
      ...
      if (!token) {
        console.log(`[OAUTH2-AUTH] ❌ No token provided for API: ${apiPath}`);
        return res.status(401).json({ 
          error: 'unauthorized',
      ...
      try {
        ...
        console.log(`[OAUTH2-AUTH] ✅ Token validated for user: ${req.user.email}`);
        return next();
      } catch (error: any) {
        ...
        console.log(`[OAUTH2-AUTH] ❌ Token validation failed:`, error.message);
        return res.status(401).json({ 
          error: 'invalid_token',
  ...
  const enterpriseAuth = async (req: any, res: any, next: any) => {
    ...
    try {
      ...
      // SECURITY FIX: Normalize roles to lowercase for consistent case-insensitive matching
      const normalizedRoles = (decoded.roles || []).map((r: string) => r.toLowerCase());
      req.user = {
  ...
  app.get('/api/auth/session', async (req: any, res) => {
    ...
    // Check for auth token (production/oauth2 mode)
    const authHeader = req.headers.authorization;
    const token = authHeader?.split(' ')[1];

    // If no token, return null user (not 401) - frontend will show login page
    if (!token) {
    ...
    if (token === 'undefined' || token === 'null' || token === '' || token.length < 10) {
      return res.json({ user: null, authenticated: false, reason: 'invalid_token_format' });
    }

    const tokenParts = token.split('.');
    if (tokenParts.length !== 3) {
      return res.json({ user: null, authenticated: false, reason: 'malformed_token' });
    }
    ...
    try {
      ...
      res.json({ ...sessionData, authenticated: true });
    } catch (error) {
  ...
  // Permissions endpoint - returns user permissions for RBAC
  app.get('/api/auth/permissions', async (req: any, res) => {
    ...
    try {
      ...
      // Return user permissions from token or default admin permissions
      return res.json({
        permissions: decoded.permissions || ['*'],
        roles: decoded.roles || ['admin'],
        capabilities: decoded.capabilities || ['full_access'],
        userId: decoded.id || decoded.userId,
        tenantId: decoded.tenantId
      });
    } catch (error) {
      console.error("[AUTH] Permissions verification failed:", error);
    ...
    try {
      ...
      while (currentDate <= rangeEnd) {
        ...
        if (override) {
          ...
          if (isOpen) {
            const rule = rulesMap.get(dayOfWeekName as "monday" | "tuesday" | "wednesday" | "thursday" | "friday" | "saturday" | "sunday");
            if (rule) {
  ...
  app.get('/api/stores/nearby', tenantMiddleware, rbacMiddleware, requirePermission('stores.read'), async (req: any, res) => {
    ...
    try {
      ...
      try {
        ...
        // GPS coordinates are stored inside the geo JSONB field
        ...
        .where(
          and(
            ...
            // Filter only stores with valid GPS coordinates in geo JSONB
            sql`${w3suiteStores.geo}->>'latitude' IS NOT NULL AND ${w3suiteStores.geo}->>'longitude' IS NOT NULL`
          )
        )
        .limit(50);
      ...
      const nearbyStores = storeResults
        .map((store: StoreQueryResult) => {
          ...
          const storeLat = parseFloat(geoData?.latitude || '0');
          const storeLng = parseFloat(geoData?.longitude || '0');
          ...
          return {
            ...
            radius: storeRadius,
            // Include WiFi networks from geo JSONB for additional geofencing
            wifiNetworks: geoData?.wifiNetworks || []
          };
        })
  ...
  app.put('/api/users/:id', ...authWithRBAC, requirePermission('users.update'), async (req: any, res) => {
    try {
      ...
      // Create partial update schema (all fields optional except id/tenantId)
  ...
  app.get('/api/stores/:storeId/location', async (req: any, res) => {
    try {
      ...
      // Extract GPS data from geo JSONB field
      const geoData = storeData[0].geo as any;
      res.json({
        id: storeData[0].id,
        nome: storeData[0].nome,
        latitude: geoData?.latitude || null,
        longitude: geoData?.longitude || null,
        wifiNetworks: geoData?.wifiNetworks || [],
        address: storeData[0].address,
        citta: storeData[0].citta
      });
    } catch (error) {
  ...
  app.get('/api/hr/time-tracking/reports', ...authWithRBAC, async (req: any, res) => {
    try {
      ...
      const holidayMinutes = entries.filter(e => (e as any).holidayBonus).reduce((sum, e) => sum + (e.totalMinutes || 0), 0);
  ...
  app.get('/api/hr/analytics/leave', ...authWithRBAC, async (req: any, res) => {
    try {
      ...
      // Using universalRequests with department='hr' and category='leave' instead
      const leaveStats = [{
        totalRequests: 25,
        approvedRequests: 18,
        pendingRequests: 3,
      ...
      // Get leave-related universal requests
      const leaveByType = await db.select({
        ...
        .where(and(
          ...
          eq(universalRequests.category, 'leave')
        ))
        .groupBy(universalRequests.type);
  ...
  app.get('/api/requests', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    try {
      ...
      // Full-text search - use sql`` for ILIKE condition
      if (filters.search) {
        const searchPattern = `%${filters.search}%`;
        conditions.push(
          sql`(${universalRequests.title} ILIKE ${searchPattern} OR ${universalRequests.description} ILIKE ${searchPattern})`
        );
      }
  ...
  app.patch('/api/requests/:id', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    ...
    try {
      ...
      // Build update data with type-safe field iteration
      const updateData: Record<string, unknown> = {
        updatedBy: userId,
        updatedAt: new Date()
      };
      
      // Type-safe field mapping for updateUniversalRequestBodySchema
      const fieldMap = {
  ...
  app.patch('/api/requests/:id/status', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    ...
    try {
      ...
      // Build update data - 'completed' status not in enum, use 'approved' for completion
      const updateData: Record<string, unknown> = {
        status: status,
        updatedBy: userId,
        updatedAt: new Date()
      ...
      // Set approver when approved or rejected
      ...
      // ✅ AUDIT TRAIL: Log status change (without department/category in type)
      await logRequestStatusChanged({
        ...
        reason,
        duration,
        category: currentRequest[0].category || undefined,
        requestType: currentRequest[0].department || undefined
      });
  ...
  app.post('/api/hr/requests', tenantMiddleware, rbacMiddleware, requirePermission('hr.requests.create'), async (req: any, res) => {
    try {
      ...
      const request = await storage.createRequest(requestData as any);
  ...
  app.get('/api/hr/requests/:id', tenantMiddleware, rbacMiddleware, requirePermission('hr.requests.view.self'), async (req: any, res) => {
    try {
      ...
      const userPermissions = req.userPermissions || [];
      // HRRequestWithDetails uses nested requester/currentApprover objects
      const requesterId = (request as any).requester?.id || (request as any).requesterId;
      const currentApproverId = (request as any).currentApprover?.id || (request as any).currentApproverId;
      const isOwner = requesterId === userId;
      const isCurrentApprover = currentApproverId === userId;
  ...
  app.post('/api/hr/requests/:id/approve', tenantMiddleware, rbacMiddleware, requirePermission('hr.requests.approve'), async (req: any, res) => {
    try {
      ...
      try {
        const existingRequest = await storage.getRequestById(tenantId, requestId);
        // Access status via type assertion since HRRequestWithDetails extends HrRequest
        fromStatus = (existingRequest as any)?.status || null;
      } catch (error) {
  ...
  app.post('/api/hr/requests/:id/submit', tenantMiddleware, rbacMiddleware, requirePermission('hr.requests.view.self'), async (req: any, res) => {
    try {
      ...
      // HRRequestWithDetails uses nested requester object
      const requesterId = (existingRequest as any).requester?.id || (existingRequest as any).requesterId;
  ...
  app.get('/api/hr/requests/:id/comments', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    try {
      ...
      const userPermissions = req.userPermissions || [];
      const requesterId = (request as any).requester?.id || (request as any).requesterId;
      const canView = requesterId === userId || userPermissions.includes('hr.requests.approve');
  ...
  app.get('/api/calendar/events', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    try {
      ...
      // Type for calendar event filters
      const filters = {
  ...
  app.post('/api/calendar/events', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    try {
      ...
      // Calendar permission check - stub for now as getUserCalendarPermissions may not be implemented
      const eventData = validatedData as any;
      
      // Restrict HR-sensitive events to HR_MANAGER/ADMIN only
      if (eventData.hrSensitive && !['HR_MANAGER', 'ADMIN'].includes(userRole)) {
        return res.status(403).json({
      ...
      // Validate required fields for createCalendarEvent
      if (!eventData.startDate || !eventData.endDate || !eventData.title) {
        return res.status(400).json({
          error: 'validation_error',
          message: 'startDate, endDate e title sono campi obbligatori'
        });
      }
      ...
      const event = await storage.createCalendarEvent({
        ...
        recurring: eventData.recurrence,
  ...
  app.put('/api/calendar/events/:id', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    try {
      ...
      // Simplified permission check: owner or admin can update
      const isOwner = (existingEvent as any).ownerId === userId;
      const isAdmin = ['ADMIN', 'HR_MANAGER', 'STORE_MANAGER'].includes(userRole);
      ...
      const event = await storage.updateCalendarEvent(eventId, updateData, tenantId);
  ...
  app.delete('/api/calendar/events/:id', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    try {
      ...
      // Simplified permission check: owner or admin can delete
      const isOwner = (existingEvent as any).ownerId === userId;
      const isAdmin = ['ADMIN', 'HR_MANAGER', 'STORE_MANAGER'].includes(userRole);
  ...
  app.get('/api/calendar/events/:id', tenantMiddleware, rbacMiddleware, async (req: any, res) => {
    try {
      ...
      // Import CalendarScope and permissions
      const { CALENDAR_PERMISSIONS } = await import('./hr-storage.js');
      const rolePermissions = CALENDAR_PERMISSIONS[userRole as keyof typeof CALENDAR_PERMISSIONS] || CALENDAR_PERMISSIONS.USER;
      const permissions = {
        canViewScopes: rolePermissions.view || [CalendarScope.OWN],
        canViewHrSensitive: ['HR_MANAGER', 'ADMIN'].includes(userRole)
      };
  ...
  app.post('/api/ai/vectors/search', ...authWithRBAC, requirePermission('ai.vectors.search'), async (req: any, res) => {
    try {
      ...
      const results = await storage.searchSimilarVectorEmbeddings(
        tenantId,
        queryEmbedding,
        {
          limit,
  ...
  app.post('/api/ai/chat', ...authWithRBAC, requirePermission('ai.chat.use'), enforceAIEnabled, async (req: any, res) => {
    try {
      ...
      const featuresEnabled = settings.featuresEnabled as Record<string, boolean> | undefined;
      if (!featuresEnabled?.chat_assistant) {
      ...
      if (includeWebSearch) {
        const currentFeatures = (settings.featuresEnabled as Record<string, boolean>) || {};
        settings.featuresEnabled = {
          ...currentFeatures,
          web_search: true
        } as typeof settings.featuresEnabled;
      }
  ...
  app.post('/api/ai/analyze-document', ...authWithRBAC, requirePermission('ai.documents.analyze'), enforceAIEnabled, async (req: any, res) => {
    try {
      ...
      const settings = await storage.getAISettings(tenantId);
      const documentFeaturesEnabled = settings?.featuresEnabled as Record<string, boolean> | undefined;
      if (!settings || !settings.isActive || !documentFeaturesEnabled?.document_analysis) {
    ...
    try {
      ...
      if (extractContent && result.content) {
        ...
        if (embeddingResult.success && embeddingResult.embedding) {
          const contentToStore = result.content.substring(0, 5000);
          // Store the embedding
          await storage.createVectorEmbedding({
            ...
            status: 'ready',
            tokenCount: Math.ceil(contentToStore.length / 4)
          });
          ...
          await storage.createAITrainingSession({
            ...
            failedChunks: 0,
            embeddingsCreated: 1,
            tokensProcessed: embeddingResult.usage?.total_tokens || 0
          });
        ...
        for (const chunk of processingResult.chunks) {
          if (chunk.embedding) {
            ...
            const stored = await storage.createVectorEmbedding({
              ...
              embeddingModel: 'text-embedding-3-small',
              mediaType: mediaType as 'text' | 'audio' | 'video' | 'image',
              metadata: {
                ...chunk.metadata,
  ...
  app.get('/api/ai/agents/:agentId/cross-tenant-knowledge', async (req: any, res) => {
    try {
      ...
      const allSessions = await storage.getAITrainingSessions(null as any, { limit: 1000 });
      ...
      const vectorEmbeddings = await storage.getVectorCollections(null as any);
      const agentEmbeddings = vectorEmbeddings.filter(embedding => 
        embedding.name?.includes(agentId)
      );
        ...
        for (const session of docSessions) {
          if (session.sourceFile) {
            items.push({
              ...
              contentPreview: `Documento processato: ${session.sourceFile}`,
  ...
  app.get('/api/universal-requests', ...authWithRBAC, requirePermission('universal_requests.read'), async (req: any, res) => {
    try {
      // Preferisci sempre l'header X-Tenant-ID che contiene l'UUID corretto
      const tenantId = req.headers['x-tenant-id'] || req.user?.tenantId || req.tenantId;
      const filters = legacyUniversalRequestFiltersSchema.parse(req.query);
      ...
      if (filters.requestType) {
        whereConditions.push(eq(universalRequests.type, filters.requestType));
      }
      if (filters.priority) {
  ...
  app.get('/api/universal-requests/:id', ...authWithRBAC, requirePermission('universal_requests.read'), async (req: any, res) => {
    try {
      // Preferisci sempre l'header X-Tenant-ID che contiene l'UUID corretto
      const tenantId = req.headers['x-tenant-id'] || req.user?.tenantId || req.tenantId;
      const requestId = parseUUIDParam(req.params.id, 'Request ID');
      ...
      res.json({
        ...
        data: {
          ...requestData.request,
          requesterName: `${requestData.requesterFirstName || ''} ${requestData.requesterLastName || ''}`.trim(),
  ...
  app.post('/api/universal-requests/:id/ai-route', ...authWithRBAC, requirePermission('universal_requests.update'), async (req: any, res) => {
    try {
      ...
      if (!tenantAISettings || !tenantAISettings.isActive) {
        return res.status(400).json({ 
