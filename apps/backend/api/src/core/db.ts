 ...
 * 
 * Supports both call patterns for backward compatibility:
 * - setTenantContext(tenantId) 
 * - setTenantContext(db, tenantId)
 */
export const setTenantContext = async (
  firstArg: string | typeof db,
  secondArg?: string
) => {
  ...
  // CRITICAL: Must use 'app.tenant_id' to match RLS policies on VPS
  await db.execute(
...
export const withTenantContext = async <T>(
  ...
  try {
    ...
    await db.execute(sql.raw(`SELECT set_config('app.tenant_id', NULL, false)`));
  }
};
