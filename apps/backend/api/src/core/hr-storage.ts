...
export const CALENDAR_PERMISSIONS = {
  ...
  USER: {
    ...
    approveLeave: false,
    manageShifts: false,
    viewHrSensitive: false,
    manageTimeTracking: false
  },
...
export interface IHRStorage {
  ...
  getTimeTrackingForUser(userId: string, dateRange: DateRange): Promise<TimeTrackingEntryDTO[]>;
...
export class HRStorage implements IHRStorage {
  ...
  async getCalendarEvents(
    ...
    if (filters?.startDate && filters?.endDate) {
      ...
      filterConditions.push(
        or(
          between(calendarEvents.startDate, startDate, endDate),
          between(calendarEvents.endDate, startDate, endDate)
        )
      );
    }
  ...
  // Leave Requests (using universalRequests with category='leave')
  async getLeaveRequests(
    ...
    if (!permissions.canApproveLeave) {
      ...
      conditions.push(eq(leaveRequests.currentApproverId, filters.approverId));
    }
    ...
    if (filters?.leaveType) {
      // leaveType is stored in the 'type' column
      conditions.push(eq(leaveRequests.type, filters.leaveType));
    }
    ...
    if (filters?.startDate && filters?.endDate) {
      ...
      conditions.push(
        or(
          sql`${leaveRequests.startDate} BETWEEN ${startDateStr} AND ${endDateStr}`,
          sql`${leaveRequests.endDate} BETWEEN ${startDateStr} AND ${endDateStr}`
        ) as any
      );
    }
  ...
  async getShifts(tenantId: string, storeId: string, dateRange: DateRange): Promise<Shift[]> {
    ...
    if (startDate || endDate) {
      conditions.push(
        between(shifts.date, startDateStr, endDateStr)
      );
    }
    
    // Select all columns from shifts table and return as Shift[]
  ...
  async getEmployeeShifts(tenantId: string, userId: string, dateRange?: DateRange): Promise<any[]> {
    ...
    return results.map(shift => ({
      ...
      startTime: shift.startTime instanceof Date ? shift.startTime.toISOString().slice(11, 16) : String(shift.startTime).slice(0, 5), // HH:MM format
      endTime: shift.endTime instanceof Date ? shift.endTime.toISOString().slice(11, 16) : String(shift.endTime).slice(0, 5),
      status: shift.status,
  ...
  async clockOut(trackingId: string, tenantId: string, notes?: string): Promise<TimeTracking> {
    ...
    const clockIn = new Date(tracking[0].clockIn);
  ...
  private async autoMatchShiftAssignment(timeTrackingEntry: TimeTracking): Promise<void> {
    ...
    const clockIn = new Date(timeTrackingEntry.clockIn);
    const windowStart = new Date(clockIn.getTime() - TOLERANCE_MINUTES * 60000);
    const windowEnd = new Date(clockIn.getTime() + TOLERANCE_MINUTES * 60000);
    ...
    const matchingAssignments = await db.select()
      ...
      .orderBy(sql`ABS(EXTRACT(EPOCH FROM (${shiftAssignments.expectedClockIn} - ${sql`'${clockIn.toISOString()}'::timestamp`})))`)
      .limit(1); // Take only the closest match
    
    if (matchingAssignments.length === 0) {
      console.log(`[AUTO-MATCH] No matching shift assignment found for user ${timeTrackingEntry.userId} at store ${timeTrackingEntry.storeId} at ${clockIn.toISOString()}`);
      return;
    }
    ...
    const clockInDeviationMinutes = Math.floor((clockIn.getTime() - expectedClockIn.getTime()) / 60000);
    ...
    if (timeTrackingEntry.clockOut && assignment.expectedClockOut) {
      const clockOut = new Date(timeTrackingEntry.clockOut);
      const expectedClockOut = new Date(assignment.expectedClockOut);
      const clockOutDeviationMinutes = Math.floor((clockOut.getTime() - expectedClockOut.getTime()) / 60000);
      
      updateData.actualClockOut = clockOut;
  ...
  async autoScheduleShifts(tenantId: string, storeId: string, startDate: Date, endDate: Date, constraints?: any): Promise<any> {
    ...
    const result: {
      shifts: Shift[];
      conflicts: any[];
      stats: { totalShifts: number; totalHours: number; coverageRate: number; };
    } = {
      shifts: [],
      conflicts: [],
      stats: {
  ...
  async getStaffAvailability(tenantId: string, storeId: string, date: Date): Promise<any[]> {
    ...
    for (const user of staff) {
      // Check if user has leave on this date (using universalRequests with category='leave')
    ...
    try {
      ...
      for (const assignment of assignments) {
        ...
        for (const employeeId of employeeIds) {
          try {
            // Check for existing assignment using shiftAssignments (correct table for shift assignments)
            const existingAssignment = await db.select()
              .from(shiftAssignments)
              .where(and(
                eq(shiftAssignments.userId, employeeId),
            ...
            // Create assignment using shiftAssignments table
            await db.insert(shiftAssignments).values({
              tenantId,
              userId: employeeId,
              shiftId,
              assignedBy: 'system', // Bulk assignment by system
            ...
            conflicts.push({
              ...
              shiftId,
              employeeId,
              message: `Failed to assign employee: ${errorMessage}`
            });
            failed++;
          }
        }
    ...
    try {
      for (const assignment of assignments) {
        ...
        for (const employeeId of employeeIds) {
          ...
          const shiftDate = (shiftData as any).date;
          const shiftDateStr = typeof shiftDate === 'string' ? shiftDate : new Date(shiftDate).toISOString().split('T')[0];
          const blockingAvailability = await db.select()
            ...
            .where(and(
              ...
              sql`${resourceAvailability.startDate} <= ${shiftDateStr}`,
              sql`${resourceAvailability.endDate} >= ${shiftDateStr}`
            ));
  ...
  async matchTimbratureWithShifts(tenantId: string, options: any): Promise<{
    ...
    try {
      ...
      for (const shiftRecord of plannedShifts) {
        ...
        const assignment = shiftRecord.shift_assignments;
        ...
        if (matchingEntries.length === 0) {
          ...
          const timeDifference = Math.abs(clockIn.getTime() - shiftStart.getTime()) / (1000 * 60);
          ...
          if (!isOnTime) {
            discrepancies.push({
              type: 'late_arrival',
              ...result,
              message: `Employee arrived ${Math.round(timeDifference)} minutes ${clockIn > shiftStart ? 'late' : 'early'}`
            });
          }

          if (entry.clockOut) {
            const clockOut = new Date(entry.clockOut);
            const endTimeDifference = Math.abs(clockOut.getTime() - shiftEnd.getTime()) / (1000 * 60);
            
            if (endTimeDifference > 15) {
              discrepancies.push({
                type: 'early_departure',
                ...result,
                message: `Employee left ${Math.round(endTimeDifference)} minutes ${clockOut < shiftEnd ? 'early' : 'late'}`
              });
            }
          }
        }
  ...
  async generateComplianceReport(tenantId: string, options: any): Promise<any> {
    ...
    try {
      ...
      const report = {
        ...
        recommendations: [] as Array<{ type: string; priority: string; message: string; }>
      };
