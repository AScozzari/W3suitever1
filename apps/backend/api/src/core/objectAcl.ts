...
export class ObjectAclService {
  ...
  async listAccessibleObjects(
    ...
    try {
      ...
      // Query all ACLs for this tenant from database
      const allAcls = await db
        .select()
        .from(objectAcls)
        .where(eq(objectAcls.ownerTenantId, tenantId));

      for (const aclRecord of allAcls) {
        const hasAccess = await this.checkPermission(aclRecord.objectPath, userId, tenantId, permission);
        if (hasAccess) {
          accessibleObjects.push(aclRecord.objectPath);
        }
      }
