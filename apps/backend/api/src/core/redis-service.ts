...
export class RedisService {
  ...
  async getAICacheStats(tenantId: string): Promise<{
    ...
    try {
      ...
      // Get memory usage (approximation) - use STATS command for overall memory
      const allKeys = [...aiKeys, ...embeddingKeys, ...webSearchKeys, ...documentKeys];
      let totalMemoryBytes = 0;
      
      // Get memory usage for each key individually (MEMORY USAGE takes one key at a time)
      for (const key of allKeys.slice(0, 100)) { // Limit to first 100 keys for performance
        try {
          const keyMemory = await this.redis.memory('USAGE', key);
          if (typeof keyMemory === 'number') {
            totalMemoryBytes += keyMemory;
          }
        } catch {
          // Skip keys that don't exist or have other issues
        }
      }
