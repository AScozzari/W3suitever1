...
export class NotificationService {
  ...
  async sendNotification(
    ...
    title: string,
    message: string,
    typeOrOptions?: string | NotificationDeliveryOptions,
    priorityOrUndefined?: string,
    ...
    if (typeof typeOrOptions === 'object') {
      ...
      options = {
        priority: (priorityOrUndefined as 'low' | 'medium' | 'high') || 'medium'
      };
    }
    
    return this.sendDirectNotification(tenantId, targetUserId, title, message, options);
  }
  ...
  async getUserNotifications(
    ...
    try {
      ...
      // Note: limit and offset are applied above
  ...
  private async getTargetUsers(targeting: NotificationTargeting) {
    ...
    // Build conditions array
    const conditions = [eq(users.tenantId, targeting.tenantId)];
    ...
    // For now, just return all users matching conditions
  ...
  private shouldSendNotification(
    ...
    const minPriority = preferences.minPriorityInApp || 'low';
    
    if (priorityOrder[notificationPriority] < priorityOrder[minPriority]) {
      return false;
    }
  ...
  static async notifyStatusChange(
    ...
    try {
      ...
      const hrRequest = await db
        ...
        .where(eq(universalRequests.id, hrRequestId))
        .limit(1);
