  ...
  async getObjectMetadata(objectPath: string): Promise<ObjectMetadata | null> {
    try {
      ...
      if (!metadataRecord || metadataRecord.length === 0) {
        ...
        // Create metadata from ACL record (using available fields)
        const metadata: ObjectMetadata = {
          id: acl.id,
          fileName: fileName,
          ...
          publicUrl: (acl.visibility === 'public') ? this.getPublicUrl(objectPath) : undefined
        };
...
export class AvatarService {
  ...
  async getSignedDownloadUrlForAvatar(objectPath: string, userId: string, tenantId: string): Promise<string> {
    return this.objectStorage.getSignedDownloadUrl(objectPath, userId, tenantId);
  }

  async getSignedPreviewUrlForAvatar(objectPath: string, userId: string, tenantId: string): Promise<string> {
    return this.objectStorage.getSignedPreviewUrl(objectPath, userId, tenantId);
  }
