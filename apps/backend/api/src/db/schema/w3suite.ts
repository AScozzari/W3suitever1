...
export const stores = w3suiteSchema.table("stores", {
  ...
  hasWarehouse: boolean("has_warehouse").default(true).notNull(), // Default true for sales_point/warehouse, false for office
...
// ==================== TENANT DRIVER CATEGORIES (extend brand drivers with tenant-specific categories) ====================
// NOTE: Drivers use single-table pattern with 'source' column (brand/tenant)
// Tenant can only ADD categories to brand drivers, never modify brand drivers themselves
export const tenantDriverCategories = w3suiteSchema.table("tenant_driver_categories", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  brandDriverId: uuid("brand_driver_id").notNull(), // References w3suite.drivers (brand driver)
  code: varchar("code", { length: 50 }).notNull(),
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  ...
  uniqueIndex("tenant_driver_categories_unique").on(table.tenantId, table.brandDriverId, table.code),
]);
...
// ==================== TENANT DRIVER TYPOLOGIES (extend tenant categories with tenant-specific typologies) ====================
// NOTE: Typologies can be added to tenant driver categories
export const tenantDriverTypologies = w3suiteSchema.table("tenant_driver_typologies", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  tenantCategoryId: uuid("tenant_category_id").notNull().references(() => tenantDriverCategories.id, { onDelete: 'cascade' }),
  code: varchar("code", { length: 50 }).notNull(),
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  active: boolean("active").default(true),
  ...
  index("idx_tenant_driver_typologies_tenant_category").on(table.tenantCategoryId),
  uniqueIndex("tenant_driver_typologies_unique").on(table.tenantId, table.tenantCategoryId, table.code),
]);
...
// Backward-compatible alias for legacy code using leaveRequests
export const leaveRequests = universalRequests;
export type LeaveRequest = UniversalRequest;
export type InsertLeaveRequest = InsertUniversalRequest;
export const insertLeaveRequestSchema = insertUniversalRequestSchema;
...
// ==================== HR REQUEST COMMENTS ====================
// Comments and discussions on HR requests
export const hrRequestComments = w3suiteSchema.table("hr_request_comments", {
  ...
  isInternal: boolean("is_internal").default(false), // Internal comments visible only to HR/managers
  ...
  index("hr_request_comments_created_idx").on(table.tenantId, table.requestId, table.createdAt),
]);
...
export const hrRequestCommentsRelations = relations(hrRequestComments, ({ one }) => ({
  ...
  request: one(universalRequests, { fields: [hrRequestComments.requestId], references: [universalRequests.id] }),
  author: one(users, { fields: [hrRequestComments.authorId], references: [users.id] }),
}));

// ==================== HR REQUEST APPROVALS ====================
// Approval decisions and workflow tracking for HR requests
export const hrRequestApprovals = w3suiteSchema.table("hr_request_approvals", {
  ...
  approverLevel: integer("approver_level").default(1), // Multi-level approval support
  action: varchar("action", { length: 20 }).notNull(), // approved, rejected, escalated, delegated
  comment: text("comment"), // Approval/rejection comment
  ...
  index("hr_request_approvals_tenant_approver_created_idx").on(table.tenantId, table.approverId, table.createdAt.desc()),
]);
...
export const hrRequestApprovalsRelations = relations(hrRequestApprovals, ({ one }) => ({
  ...
  request: one(universalRequests, { fields: [hrRequestApprovals.requestId], references: [universalRequests.id] }),
  approver: one(users, { fields: [hrRequestApprovals.approverId], references: [users.id] }),
}));

// ==================== HR REQUEST STATUS HISTORY ====================
// Audit trail for status transitions on HR requests
export const hrRequestStatusHistory = w3suiteSchema.table("hr_request_status_history", {
  ...
  fromStatus: varchar("from_status", { length: 30 }), // null for initial creation
...
export const hrRequestStatusHistoryRelations = relations(hrRequestStatusHistory, ({ one }) => ({
  ...
  request: one(universalRequests, { fields: [hrRequestStatusHistory.requestId], references: [universalRequests.id] }),
...
export const voipExtensions = w3suiteSchema.table("voip_extensions", {
  ...
  wsPort: integer("ws_port").default(443),
  wsPath: varchar("ws_path", { length: 100 }).default('/ws'),
...
// 1) wmsCategories - Product categories (Brand-Tenant hybrid architecture) with VERSIONING
export const wmsCategories = w3suiteSchema.table("wms_categories", {
  ...
  // VERSIONING SYSTEM - Immutable updates: edits create new versions, old versions archived
  versionNumber: integer("version_number").default(1).notNull(), // Increments on each update
  originalId: varchar("original_id", { length: 100 }), // Points to first version ID (null for v1)
  validFrom: timestamp("valid_from").defaultNow().notNull(), // When this version became active
  validTo: timestamp("valid_to"), // null = current version; set when superseded
  ...
  index("wms_categories_version_idx").on(table.tenantId, table.originalId, table.versionNumber), // Version history queries
  index("wms_categories_current_idx").on(table.tenantId, table.validTo), // Fast lookup for current versions (validTo IS NULL)
]);
...
// 2) wmsProductTypes - Product types within categories (Brand-Tenant hybrid architecture) with VERSIONING
export const wmsProductTypes = w3suiteSchema.table("wms_product_types", {
  ...
  index("wms_types_version_idx").on(table.tenantId, table.originalId, table.versionNumber), // Version history queries
  index("wms_types_current_idx").on(table.tenantId, table.validTo), // Fast lookup for current versions (validTo IS NULL)
...
export const products = w3suiteSchema.table("products", {
  ...
  imei1: varchar("imei1", { length: 20 }), // IMEI 1 (15-17 digits)
  imei2: varchar("imei2", { length: 20 }), // IMEI 2 (15-17 digits) - dual SIM
  imei3: varchar("imei3", { length: 20 }), // IMEI 3 (15-17 digits) - eSIM or additional
...
export const productItems = w3suiteSchema.table("product_items", {
  ...
  // Batch tracking (groups items from same purchase order)
  // Note: FK to product_batches added via foreignKey() in indexes section (circular dependency)
  ...
  currentHolderId: uuid("current_holder_id").references(() => crmCustomers.id, { onDelete: 'set null' }), // Customer if sold
  
  // Location
  warehouseLocation: varchar("warehouse_location", { length: 100 }), // Physical location code
  
  // Version reference for price accuracy in reports
  productVersionId: uuid("product_version_id"), // FK added after productVersions table creation
  
  // Lifecycle tracking
  orderId: uuid("order_id"), // FK to orders table (future)
  customerId: uuid("customer_id"), // FK to customers table (legacy, use currentHolderId)
  ...
  index("product_items_batch_idx").on(table.batchId),
  index("product_items_supplier_idx").on(table.lastSupplierId),
  index("product_items_holder_idx").on(table.currentHolderId),
  index("product_items_version_idx").on(table.productVersionId),
]);
...
export const wmsStockMovements = w3suiteSchema.table("wms_stock_movements", {
  ...
  productVersionId: uuid("product_version_id"), // FK to product_versions - price at time of movement
  ...
  // CQRS: Status transition tracking
  fromStatus: productLogisticStatusEnum("from_status"), // Previous logistic status
  toStatus: productLogisticStatusEnum("to_status"), // New logistic status
  
  // CQRS: Document reference (typed for FK consistency)
  documentType: varchar("document_type", { length: 50 }), // 'purchase_order', 'sale', 'transfer', 'return', 'adjustment'
  documentId: uuid("document_id"), // UUID reference to document table
  
  // Economic tracking
  unitCost: numeric("unit_cost", { precision: 10, scale: 2 }), // Cost at time of movement
  unitPrice: numeric("unit_price", { precision: 10, scale: 2 }), // Price at time of movement
  
  // Legacy external references (keep for backwards compatibility)
...
export const productBatches = w3suiteSchema.table("product_batches", {
  ...
  index("product_batches_supplier_id_idx").on(table.supplierId),
  index("product_batches_purchase_order_idx").on(table.purchaseOrderId),
...
// ==================== WMS CQRS ARCHITECTURE ====================

// WMS Enums for documents
export const versionChangeReasonEnum = pgEnum('version_change_reason', ['correction', 'business_change']);
export const purchaseOrderStatusEnum = pgEnum('purchase_order_status', ['draft', 'confirmed', 'partial', 'received', 'cancelled']);
export const wmsSaleStatusEnum = pgEnum('wms_sale_status', ['draft', 'confirmed', 'completed', 'cancelled']);
export const wmsTransferStatusEnum = pgEnum('wms_transfer_status', ['draft', 'in_transit', 'received', 'cancelled']);
export const wmsReturnStatusEnum = pgEnum('wms_return_status', ['pending', 'approved', 'received', 'processed', 'rejected']);
export const wmsReturnTypeEnum = pgEnum('wms_return_type', ['customer_return', 'doa_return', 'supplier_return']);
export const wmsReturnItemConditionEnum = pgEnum('wms_return_item_condition', ['like_new', 'damaged', 'defective']);
export const wmsDocumentTypeEnum = pgEnum('wms_document_type', ['purchase_order', 'sale', 'transfer', 'return', 'adjustment']);
export const wmsTransferItemStatusEnum = pgEnum('wms_transfer_item_status', ['pending', 'shipped', 'received']);

// 8) product_versions - Price/Fee versioning for products (immutable commercial data)
export const productVersions = w3suiteSchema.table("product_versions", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  productId: varchar("product_id", { length: 100 }).notNull(),
  
  // Version tracking
  version: integer("version").notNull().default(1), // Incremental: 1, 2, 3...
  
  // Commercial data (what can change over time)
  monthlyFee: numeric("monthly_fee", { precision: 10, scale: 2 }), // Canone mensile € (for CANVAS)
  unitPrice: numeric("unit_price", { precision: 10, scale: 2 }), // Prezzo vendita €
  costPrice: numeric("cost_price", { precision: 10, scale: 2 }), // Costo base €
  
  // Validity period
  validFrom: date("valid_from").notNull(), // Start date of this version
  validTo: date("valid_to"), // End date (NULL = current active version)
  
  // Change metadata
  changeReason: versionChangeReasonEnum("change_reason").notNull(), // correction | business_change
  changeNotes: text("change_notes"), // Reason for change
  
  // Audit
  createdBy: varchar("created_by").references(() => users.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  // Only one active version per product (validTo IS NULL)
  uniqueIndex("product_versions_active_unique").on(table.tenantId, table.productId).where(sql`valid_to IS NULL`),
  
  // Version number uniqueness per product
  uniqueIndex("product_versions_version_unique").on(table.tenantId, table.productId, table.version),
  
  // Performance indexes
  index("product_versions_tenant_idx").on(table.tenantId),
  index("product_versions_tenant_product_idx").on(table.tenantId, table.productId),
  index("product_versions_valid_from_idx").on(table.validFrom),
  index("product_versions_valid_to_idx").on(table.validTo),
  
  // Composite FK: product_versions → products
  foreignKey({
...
export const insertProductVersionSchema = createInsertSchema(productVersions).omit({
  ...
  validTo: z.coerce.date().optional(),
  changeReason: z.enum(['correction', 'business_change']),
...
// 9) wms_inventory_balances - Real-time inventory read model (CQRS pattern)
export const wmsInventoryBalances = w3suiteSchema.table("wms_inventory_balances", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  storeId: uuid("store_id").notNull().references(() => stores.id, { onDelete: 'cascade' }),
  productId: varchar("product_id", { length: 100 }).notNull(),
  ...
  // Economic value (at current cost)
  totalValue: numeric("total_value", { precision: 14, scale: 2 }).default('0').notNull(),
  
  // Tracking
  lastMovementAt: timestamp("last_movement_at"),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => [
  // One balance record per product per store
  uniqueIndex("wms_balances_tenant_store_product_unique").on(table.tenantId, table.storeId, table.productId),
  
  // Performance indexes
  index("wms_balances_tenant_idx").on(table.tenantId),
  index("wms_balances_tenant_store_idx").on(table.tenantId, table.storeId),
  index("wms_balances_product_idx").on(table.productId),
  index("wms_balances_available_idx").on(table.quantityAvailable),
  
  // Composite FK: wms_inventory_balances → products
  foreignKey({
    columns: [table.tenantId, table.productId],
    foreignColumns: [products.tenantId, products.id],
  }).onDelete('cascade'),
...
export type WmsInventoryBalance = typeof wmsInventoryBalances.$inferSelect;

// 10) wms_inventory_snapshots - Periodic snapshots for historical reporting (twice daily)
export const wmsInventorySnapshots = w3suiteSchema.table("wms_inventory_snapshots", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  storeId: uuid("store_id").notNull().references(() => stores.id, { onDelete: 'cascade' }),
  productId: varchar("product_id", { length: 100 }).notNull(),
  
  // Snapshot timestamp (12:00 or 23:00 daily)
  snapshotAt: timestamp("snapshot_at").notNull(),
  
  // Quantities at snapshot time
  quantityAtTime: integer("quantity_at_time").notNull(),
  reservedAtTime: integer("reserved_at_time").default(0).notNull(),
  
  // Economic value at snapshot
  valueAtTime: numeric("value_at_time", { precision: 14, scale: 2 }).default('0').notNull(),
  
  // Version reference for price accuracy
  productVersionId: uuid("product_version_id").references(() => productVersions.id, { onDelete: 'set null' }),
  
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  // One snapshot per product/store/time
  uniqueIndex("wms_snapshots_tenant_store_product_time_unique").on(
    table.tenantId, 
    table.storeId, 
    table.productId, 
    table.snapshotAt
  ...
  // Performance indexes (tenant_id first for RLS)
  index("wms_snapshots_tenant_idx").on(table.tenantId),
  index("wms_snapshots_tenant_store_idx").on(table.tenantId, table.storeId),
  index("wms_snapshots_tenant_snapshot_at_idx").on(table.tenantId, table.snapshotAt.desc()),
  index("wms_snapshots_product_idx").on(table.productId),
  
  // Composite FK: wms_inventory_snapshots → products
  foreignKey({
    columns: [table.tenantId, table.productId],
    foreignColumns: [products.tenantId, products.id],
  }).onDelete('cascade'),
...
export const purchaseOrders = w3suiteSchema.table("purchase_orders", {
  ...
  orderNumber: varchar("order_number", { length: 50 }).notNull(), // DDT number or invoice
  supplierId: uuid("supplier_id").references(() => suppliers.id, { onDelete: 'set null' }),
  ...
  uniqueIndex("purchase_orders_tenant_number_unique").on(table.tenantId, table.orderNumber),
  
  // Performance indexes
  index("purchase_orders_tenant_idx").on(table.tenantId),
  index("purchase_orders_tenant_store_idx").on(table.tenantId, table.storeId),
  ...
  index("purchase_orders_order_date_idx").on(table.orderDate.desc()),
]);
...
export const insertPurchaseOrderSchema = createInsertSchema(purchaseOrders).omit({
  ...
  orderNumber: z.string().min(1, "Numero ordine obbligatorio").max(50),
...
export const purchaseOrderItems = w3suiteSchema.table("purchase_order_items", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  // For serialized products (after receiving)
  ...
  index("po_items_order_idx").on(table.purchaseOrderId),
  index("po_items_tenant_idx").on(table.tenantId),
  index("po_items_product_idx").on(table.productId),
  index("po_items_batch_idx").on(table.batchId),
  index("po_items_item_idx").on(table.productItemId),
  
  // Composite FK: purchase_order_items → products
  foreignKey({
    columns: [table.tenantId, table.productId],
    foreignColumns: [products.tenantId, products.id],
  }).onDelete('cascade'),
...
export const insertPurchaseOrderItemSchema = createInsertSchema(purchaseOrderItems).omit({
  ...
  quantityOrdered: z.number().int().min(1, "Quantità deve essere almeno 1"),
  unitCost: z.coerce.number().min(0, "Costo deve essere >= 0"),
  notes: z.string().optional(),
});
export type InsertPurchaseOrderItem = z.infer<typeof insertPurchaseOrderItemSchema>;
export type PurchaseOrderItem = typeof purchaseOrderItems.$inferSelect;

// 13) wms_sales - Sales documents (receipts, invoices to customers)
export const wmsSales = w3suiteSchema.table("wms_sales", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  storeId: uuid("store_id").references(() => stores.id, { onDelete: 'set null' }),
  
  // Document identification
  saleNumber: varchar("sale_number", { length: 50 }).notNull(), // Receipt or invoice number
  customerId: uuid("customer_id").references(() => crmCustomers.id, { onDelete: 'set null' }),
  
  // Status & dates
  status: wmsSaleStatusEnum("status").default('draft').notNull(),
  ...
  // Unique sale number per tenant
  uniqueIndex("wms_sales_tenant_number_unique").on(table.tenantId, table.saleNumber),
  
  // Performance indexes
  index("wms_sales_tenant_idx").on(table.tenantId),
  index("wms_sales_tenant_store_idx").on(table.tenantId, table.storeId),
  index("wms_sales_tenant_status_idx").on(table.tenantId, table.status),
  index("wms_sales_customer_idx").on(table.customerId),
  index("wms_sales_sale_date_idx").on(table.saleDate.desc()),
]);

export const insertWmsSaleSchema = createInsertSchema(wmsSales).omit({
  id: true,
  tenantId: true,
  createdAt: true,
  updatedAt: true,
  ...
  saleNumber: z.string().min(1, "Numero vendita obbligatorio").max(50),
...
export type InsertWmsSale = z.infer<typeof insertWmsSaleSchema>;
export type WmsSale = typeof wmsSales.$inferSelect;

// 14) wms_sale_items - Line items for sales
export const wmsSaleItems = w3suiteSchema.table("wms_sale_items", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  saleId: uuid("sale_id").notNull().references(() => wmsSales.id, { onDelete: 'cascade' }),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  ...
  index("sale_items_batch_idx").on(table.batchId),
  index("sale_items_item_idx").on(table.productItemId),
  
  // Composite FK: wms_sale_items → products
  foreignKey({
    columns: [table.tenantId, table.productId],
    foreignColumns: [products.tenantId, products.id],
  }).onDelete('cascade'),
...
export const insertWmsSaleItemSchema = createInsertSchema(wmsSaleItems).omit({
  id: true,
  tenantId: true,
  createdAt: true,
  totalPrice: true,
  ...
  unitPrice: z.coerce.number().min(0, "Prezzo deve essere >= 0"),
  discount: z.coerce.number().min(0).optional(),
  notes: z.string().optional(),
});
export type InsertWmsSaleItem = z.infer<typeof insertWmsSaleItemSchema>;
export type WmsSaleItem = typeof wmsSaleItems.$inferSelect;

// 15) wms_transfers - Inter-store transfers
export const wmsTransfers = w3suiteSchema.table("wms_transfers", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  ...
  sourceStoreId: uuid("source_store_id").notNull().references(() => stores.id, { onDelete: 'cascade' }),
  destinationStoreId: uuid("destination_store_id").notNull().references(() => stores.id, { onDelete: 'cascade' }),
  
  // Status & dates
  status: wmsTransferStatusEnum("status").default('draft').notNull(),
  ...
  uniqueIndex("wms_transfers_tenant_number_unique").on(table.tenantId, table.transferNumber),
  
  // Performance indexes
  index("wms_transfers_tenant_idx").on(table.tenantId),
  index("wms_transfers_source_idx").on(table.sourceStoreId),
  index("wms_transfers_dest_idx").on(table.destinationStoreId),
  index("wms_transfers_status_idx").on(table.status),
  index("wms_transfers_created_idx").on(table.createdAt.desc()),
]);

export const insertWmsTransferSchema = createInsertSchema(wmsTransfers).omit({
  id: true,
  tenantId: true,
  createdAt: true,
  updatedAt: true,
  ...
  transferNumber: z.string().min(1, "Numero trasferimento obbligatorio").max(50),
  sourceStoreId: z.string().uuid("Store origine non valido"),
  destinationStoreId: z.string().uuid("Store destinazione non valido"),
  status: z.enum(['draft', 'in_transit', 'received', 'cancelled']).optional(),
  notes: z.string().optional(),
});
export type InsertWmsTransfer = z.infer<typeof insertWmsTransferSchema>;
export type WmsTransfer = typeof wmsTransfers.$inferSelect;

// 16) wms_transfer_items - Line items for transfers
export const wmsTransferItems = w3suiteSchema.table("wms_transfer_items", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  transferId: uuid("transfer_id").notNull().references(() => wmsTransfers.id, { onDelete: 'cascade' }),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  ...
  // Item status
  status: wmsTransferItemStatusEnum("status").default('pending').notNull(),
  ...
  index("transfer_items_transfer_idx").on(table.transferId),
  index("transfer_items_tenant_idx").on(table.tenantId),
  index("transfer_items_product_idx").on(table.productId),
  index("transfer_items_batch_idx").on(table.batchId),
  index("transfer_items_item_idx").on(table.productItemId),
  index("transfer_items_status_idx").on(table.status),
  
  // Composite FK: wms_transfer_items → products
  foreignKey({
    columns: [table.tenantId, table.productId],
    foreignColumns: [products.tenantId, products.id],
  }).onDelete('cascade'),
...
export const insertWmsTransferItemSchema = createInsertSchema(wmsTransferItems).omit({
  id: true,
  tenantId: true,
  createdAt: true,
}).extend({
  quantity: z.number().int().min(1, "Quantità deve essere almeno 1"),
  status: z.enum(['pending', 'shipped', 'received']).optional(),
  notes: z.string().optional(),
});
export type InsertWmsTransferItem = z.infer<typeof insertWmsTransferItemSchema>;
export type WmsTransferItem = typeof wmsTransferItems.$inferSelect;

// 17) wms_returns - Return documents (customer, DOA, supplier)
export const wmsReturns = w3suiteSchema.table("wms_returns", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  storeId: uuid("store_id").references(() => stores.id, { onDelete: 'set null' }),
  ...
  returnType: wmsReturnTypeEnum("return_type").notNull(),
  
  // Related documents
  relatedSaleId: uuid("related_sale_id").references(() => wmsSales.id, { onDelete: 'set null' }),
  relatedPurchaseOrderId: uuid("related_purchase_order_id").references(() => purchaseOrders.id, { onDelete: 'set null' }),
  ...
  // Status & dates
  status: wmsReturnStatusEnum("status").default('pending').notNull(),
  ...
  // Unique return number per tenant
  uniqueIndex("wms_returns_tenant_number_unique").on(table.tenantId, table.returnNumber),
  
  // Performance indexes
  index("wms_returns_tenant_idx").on(table.tenantId),
  index("wms_returns_tenant_store_idx").on(table.tenantId, table.storeId),
  index("wms_returns_type_idx").on(table.returnType),
  index("wms_returns_status_idx").on(table.status),
  index("wms_returns_customer_idx").on(table.customerId),
  index("wms_returns_supplier_idx").on(table.supplierId),
  index("wms_returns_return_date_idx").on(table.returnDate.desc()),
]);

export const insertWmsReturnSchema = createInsertSchema(wmsReturns).omit({
  id: true,
  tenantId: true,
  createdAt: true,
  updatedAt: true,
}).extend({
  returnNumber: z.string().min(1, "Numero reso obbligatorio").max(50),
  returnType: z.enum(['customer_return', 'doa_return', 'supplier_return']),
  returnDate: z.coerce.date().optional(),
  status: z.enum(['pending', 'approved', 'received', 'processed', 'rejected']).optional(),
  reason: z.string().optional(),
  notes: z.string().optional(),
});
export type InsertWmsReturn = z.infer<typeof insertWmsReturnSchema>;
export type WmsReturn = typeof wmsReturns.$inferSelect;

// 18) wms_return_items - Line items for returns
export const wmsReturnItems = w3suiteSchema.table("wms_return_items", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  returnId: uuid("return_id").notNull().references(() => wmsReturns.id, { onDelete: 'cascade' }),
  tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  ...
  // Item condition
  condition: wmsReturnItemConditionEnum("condition").default('like_new').notNull(),
  ...
  index("return_items_product_idx").on(table.productId),
  index("return_items_batch_idx").on(table.batchId),
  index("return_items_item_idx").on(table.productItemId),
  index("return_items_condition_idx").on(table.condition),
  
  // Composite FK: wms_return_items → products
  foreignKey({
    columns: [table.tenantId, table.productId],
    foreignColumns: [products.tenantId, products.id],
  }).onDelete('cascade'),
...
export const insertWmsReturnItemSchema = createInsertSchema(wmsReturnItems).omit({
  id: true,
  tenantId: true,
  createdAt: true,
}).extend({
  quantity: z.number().int().min(1, "Quantità deve essere almeno 1"),
  condition: z.enum(['like_new', 'damaged', 'defective']).optional(),
  notes: z.string().optional(),
});
