...
export async function seedCrmPipelines() {
  ...
  try {
    ...
    const leads = await db.insert(crmLeads).values(demoLeadsData as any).onConflictDoNothing().returning();
    ...
    await db.insert(crmDeals).values(demoDeals as any).onConflictDoNothing();
