  ...
  try {
    ...
    if (cachedTenant && isCacheValid(cachedTenant)) {
      ...
      tenant = {
        ...
        status: tenantResult[0].status || 'active'
      };
...
export async function rbacMiddleware(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // SECURITY FIX: Case-insensitive check to handle 'Admin', 'admin', 'ADMIN' variations
    const hasAdminRole = req.user.roles?.some((r: string) => r.toLowerCase() === 'admin');
    if (!rbacEnabled && hasAdminRole) {
      req.userPermissions = ['*'];
