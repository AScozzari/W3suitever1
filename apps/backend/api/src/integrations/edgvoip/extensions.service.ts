  ...
  try {
    ...
    const [created] = await db.insert(voipExtensions)
      .values({
        ...
        sipServer: 'demo.edgvoip.it',
        sipPort: 5060,
        wsPort: 443, // SEMPRE 443 per WSS
        wsPath: '/ws', // SEMPRE /ws
        transport: 'WSS',
        authRealm: 'demo.edgvoip.it',
        status: 'active',
        type: data.type || 'user',
        settings: data.settings || {
          voicemail_enabled: true,
...
export async function getUserCredentials(
  ...
  try {
    ...
    // Decrypt password (with plaintext fallback for development)
    let decryptedPassword: string | null = null;
    
    // Check if password is encrypted (format: keyId:iv:authTag:encrypted)
    const passwordParts = extension.sipPassword?.split(':') || [];
    if (passwordParts.length >= 3) {
      // Encrypted format - decrypt it
      decryptedPassword = await encryptionService.decrypt(extension.sipPassword, tenantId);
    } else if (extension.sipPassword) {
      // Plaintext fallback for development
      logger.info('SIP password is plaintext (not encrypted)', { userId, tenantId });
      decryptedPassword = extension.sipPassword;
    }

    if (!decryptedPassword) {
      logger.error('Failed to get SIP password', { userId, tenantId });
      return null;
    }
    ...
    return {
      ...
      wsPort: extension.wsPort || 443,
      wsPath: (extension as any).wsPath || '/ws',
