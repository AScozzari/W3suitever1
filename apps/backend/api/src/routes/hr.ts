...
import { users, shiftTemplates, shiftTimeSlots, shiftTemplateVersions, shiftAssignments, shiftAttendance, attendanceAnomalies, shifts, universalRequests, resourceAvailability, stores, userAssignments } from '../db/schema/w3suite';
import { eq, and, gte, lte, inArray, sql, count, desc, isNull, exists, between } from 'drizzle-orm';
...
router.post('/shifts/apply-template', requirePermission('hr.shifts.manage'), async (req: Request, res: Response) => {
  try {
    ...
    const result = await hrStorage.applyShiftTemplate(templateId, storeId, start, end, tenantId);
...
router.post('/shifts/:id/unassign', requirePermission('hr.shifts.manage'), async (req: Request, res: Response) => {
  try {
    ...
    const results = [];
    for (const uid of userIds) {
      const result = await hrStorage.removeUserFromShift(shiftId, uid);
      results.push(result);
    }
  ...
  try {
      ...
      for (const date of dates) {
        try {
          ...
          const [newShift] = await db.insert(shifts).values({
            ...
            name: template.name,
            shiftType: 'full_day',
            requiredStaff: template.defaultRequiredStaff || 1,
            startTime: new Date(`2000-01-01T${template.defaultStartTime || '09:00'}:00`),
            endTime: new Date(`2000-01-01T${template.defaultEndTime || '18:00'}:00`),
    ...
    try {
      for (const shift of createdShifts) {
        await webSocketService.broadcastShiftUpdate(tenantId, 'shift_created', {
          shiftId: shift.id,
...
router.get('/employee/my-shifts', async (req: Request, res: Response) => {
  try {
    ...
    const conditions: any[] = [
      ...
      sql`EXISTS (
        SELECT 1 FROM w3suite.shift_assignments sa
        WHERE sa.shift_id = ${shifts.id}
          AND sa.user_id = ${userId}
          AND sa.tenant_id = ${tenantId}::uuid
    ...
    if (startDate && endDate) {
      conditions.push(
        sql`${shifts.date} BETWEEN ${new Date(startDate as string)} AND ${new Date(endDate as string)}`
      );
    }
    ...
    const myShifts = await db.select({
      ...
      assignedAt: shiftAssignments.assignedAt,
      performanceNotes: shiftAssignments.performanceNotes,
      status: shiftAssignments.status
    })
    .from(shifts)
    .leftJoin(stores, eq(shifts.storeId, stores.id))
    ...
    const transformedShifts = myShifts.map(shift => ({
      ...
      role: 'Dipendente',
      status: shift.status === 'assigned' ? 'confirmed' : 'scheduled',
      title: shift.name,
      description: shift.performanceNotes,
      notes: shift.performanceNotes,
...
router.delete('/shift-templates/:id', requirePermission('hr.shifts.manage'), async (req: Request, res: Response) => {
  try {
    ...
    // Delete template using existing storage function
    await hrStorage.deleteShiftTemplate(templateId, tenantId);
    ...
    for (const assignment of assignments) {
      ...
      if ((checkAttendance.rows || []).length === 0) {
        ...
        if ((checkAnomaly.rows || []).length === 0) {
          ...
          const shiftStartTime = new Date((assignment as any).start_time);
...
router.get('/attendance/store-coverage', requirePermission('hr.shifts.read'), async (req: Request, res: Response) => {
  try {
    ...
    const shiftsData = await db.execute<{
      id: string;
      storeId: string;
      name: string;
      startTime: string;
    ...
    const coverage = (shiftsData.rows || []).map((shift) => {
  ...
  try {
    ...
    conflictingAssignments.forEach(assignment => {
      if (assignment.startTime && assignment.endTime) {
        const startTimeStr = assignment.startTime instanceof Date 
          ? `${assignment.startTime.getHours()}:${String(assignment.startTime.getMinutes()).padStart(2, '0')}`
          : String(assignment.startTime);
        const endTimeStr = assignment.endTime instanceof Date 
          ? `${assignment.endTime.getHours()}:${String(assignment.endTime.getMinutes()).padStart(2, '0')}`
          : String(assignment.endTime);
        const start = parseInt(startTimeStr.split(':')[0]) * 60 + parseInt(startTimeStr.split(':')[1] || '0');
        const end = parseInt(endTimeStr.split(':')[0]) * 60 + parseInt(endTimeStr.split(':')[1] || '0');
        ...
        coverageGaps.push({
          ...
          date: typeof assignment.shiftDate === 'string' ? assignment.shiftDate : (assignment.shiftDate as Date).toISOString().split('T')[0],
...
router.get('/stores/:id/planning-summary', requirePermission('hr.shifts.read'), async (req: Request, res: Response) => {
  try {
    ...
    const totalRequiredStaff = shiftsData.reduce((sum, s) => sum + (s.requiredStaff || 1), 0);
    const totalAssignedResources = assignmentsData.length;
    const coveragePercentage = totalRequiredStaff > 0 
      ? Math.round((totalAssignedResources / totalRequiredStaff) * 100) 
      : 0;
    ...
    const dailyBreakdown = shiftsData.reduce((acc: any, shift) => {
      ...
      acc[dateKey].requiredStaff += (shift.requiredStaff || 1);
      return acc;
    }, {});
    ...
    const gaps = shiftsData.filter(shift => {
      ...
      return shiftAssignmentCount < (shift.requiredStaff || 1);
    }).map(shift => ({
      shiftId: shift.id,
      date: shift.date,
      startTime: shift.startTime,
      endTime: shift.endTime,
      required: shift.requiredStaff || 1,
      assigned: assignmentsData.filter(a => a.shiftId === shift.id).length,
      missing: (shift.requiredStaff || 1) - assignmentsData.filter(a => a.shiftId === shift.id).length
    }));
...
router.get('/employees/:id/availability', requirePermission('hr.employees.read'), async (req: Request, res: Response) => {
  try {
    ...
    // Calculate hours worked (startTime and endTime are Date objects from shifts table)
    const calculateHours = (assignments: any[]) => {
      return assignments.reduce((total, a) => {
        if (a.startTime && a.endTime) {
          const startDate = a.startTime instanceof Date ? a.startTime : new Date(a.startTime);
          const endDate = a.endTime instanceof Date ? a.endTime : new Date(a.endTime);
    ...
    const storeHistory = assignmentsData.reduce((acc: any, a) => {
      if (a.storeId && !acc[a.storeId]) {
        acc[a.storeId] = {
          ...
          storeName: a.storeName,
          shiftsCount: 0,
          lastWorked: a.shiftDate || new Date().toISOString(),
        };
      }
      if (a.storeId && a.shiftDate) {
    ...
    const assignmentsByDate = assignmentsData.reduce((acc: any, a) => {
      if (!a.shiftDate) return acc;
      const dateKey = typeof a.shiftDate === 'string' ? a.shiftDate.split('T')[0] : String(a.shiftDate);
