...
import { getTenantAIStatus } from '../middleware/ai-enforcement';
...
// ==================== AI STATUS (NO AUTH REQUIRED - just status check) ====================

/**
 * GET /api/ai/status
 * Get AI status for the current tenant (enabled/disabled with reason)
 * No errors - always returns status object
 */
router.get('/status', async (req, res) => {
  try {
    ...
    logger.info('AI status check', { tenantId, enabled: status.enabled, reason: status.reason });
 ...
 */
router.get('/settings', rbacMiddleware, requirePermission('ai.settings.view'), async (req, res) => {
  try {
    ...
    // If no settings exist, return default disabled state (NO ERROR)
    if (!settings) {
      ...
      return res.status(200).json({
        ...
        message: 'Default AI settings (not configured yet)',
        isDefault: true,
        timestamp: new Date().toISOString()
      } as ApiSuccessResponse<typeof defaultSettings>);
    }
 ...
 */
router.put('/settings', rbacMiddleware, requirePermission('ai.settings.manage'), async (req, res) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string || req.user?.tenantId;
    if (!tenantId) {
      return res.status(400).json({
    ...
    if (existing) {
      ...
      // INSERT new settings (API key NOT required - can be added later)
      // AI will remain disabled until both isActive=true AND apiKey is configured
      [result] = await db
        .insert(aiSettings)
        .values({
          tenantId,
          openaiApiKey: data.openaiApiKey || null, // Optional at creation
          openaiModel: (data.openaiModel || 'gpt-4-turbo') as any,
          isActive: data.isActive !== undefined ? data.isActive : false, // Default to disabled
 ...
 */
router.post('/test-connection', rbacMiddleware, requirePermission('ai.settings.manage'), async (req, res) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string || req.user?.tenantId;
    if (!tenantId) {
      return res.status(400).json({
 ...
 */
router.get('/agents', rbacMiddleware, requirePermission('ai.settings.view'), async (req, res) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string || req.user?.tenantId;
    if (!tenantId) {
      return res.status(400).json({
    ...
    // Query active agents from Brand Interface central registry using main db connection
    // Note: Using static import instead of dynamic to work in bundled VPS environment
    const registryAgents = await db
      .select()
    ...
    const agentsWithTenantSettings = registryAgents.map((agent: any) => ({
      ...agent,
    ...
    logger.info('AI agents retrieved from Brand Interface registry', { 
      ...
      agents: agentsWithTenantSettings.map((a: any) => a.agentId)
    });
...
router.get('/agents/:agentId', async (req, res) => {
  try {
    ...
    // Query agent from Brand Interface registry using main db connection
    // Note: Using static import instead of dynamic to work in bundled VPS environment
 ...
 */
router.put('/agents/:agentId/toggle', rbacMiddleware, requirePermission('ai.settings.manage'), async (req, res) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string || req.user?.tenantId;
    if (!tenantId) {
      return res.status(400).json({
