...
router.get('/trunks', rbacMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch VoIP trunks' });
  }
});
...
router.post('/trunks/refresh', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to refresh trunks' });
  }
});
...
router.post('/trunks/push', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to push trunks' });
  }
});
...
router.get('/trunks/:id/status', rbacMiddleware, requirePermission('view_telephony'), async (req, res) => {
  try {
    ...
    if (!trunk.externalId) {
      return res.status(400).json({ error: 'Trunk not synced with EDGVoIP' });
    }
...
router.get('/extensions', rbacMiddleware, async (req, res) => {
  try {
    ...
    const conditions: any[] = [eq(voipExtensions.tenantId, tenantId)];
    
    // Filter by status if provided (active|inactive|suspended)
    if (status) {
      conditions.push(eq(voipExtensions.status as any, status as any));
    }
    ...
    return res.status(500).json({ error: 'Failed to fetch VoIP extensions' });
  }
});
...
router.post('/extensions', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to create VoIP extension' });
  }
});
...
router.patch('/extensions/:id', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to update VoIP extension' });
  }
});
...
router.delete('/extensions/:id', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to delete VoIP extension' });
  }
});
...
router.get('/extensions/me', rbacMiddleware, async (req, res) => {
  try {
    ...
    if (!credentials) {
      // Return 200 with null data instead of 404 - no extension is not an error
      return res.json({ 
        success: true, 
        data: null,
        message: 'No VoIP extension assigned to your account'
      });
    }
    ...
    return res.status(500).json({ error: 'Failed to retrieve SIP credentials' });
  }
});
...
router.patch('/extensions/:id/reset-password', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to reset SIP password' });
  }
});
...
router.post('/extensions/:id/sync', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to sync extension' });
  }
});
...
router.post('/extensions/push', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to push extensions' });
  }
});
  ...
  try {
    ...
    if (!result.success) {
      ...
      if (errorMessage.includes('not configured') || errorMessage.includes('API key')) {
        // EDGVoIP API not configured - return empty data with warning, not error
        logger.warn('EDGVoIP not configured for tenant', { tenantId, error: errorMessage });
        return res.json({ 
          success: true, 
          data: [],
          warning: 'VoIP esterno non configurato. Configura la API key EDGVoIP nelle impostazioni.',
          configured: false
        } as ApiSuccessResponse);
      }
      ...
      if (errorMessage.includes('Invalid API Key') || errorMessage.includes('Authentication')) {
        // API key is invalid
        logger.warn('EDGVoIP API key invalid for tenant', { tenantId });
        return res.json({ 
          success: true, 
          data: [],
          warning: 'API key EDGVoIP non valida. Verifica la configurazione.',
          configured: false
        } as ApiSuccessResponse);
      }
      
      // Network or other transient error - return warning, not 503
      logger.warn('EDGVoIP service temporarily unavailable', { tenantId, error: errorMessage });
      return res.json({ 
        success: true, 
        data: [],
        warning: 'Servizio EDGVoIP temporaneamente non raggiungibile.',
        configured: true,
    ...
    return res.json({ 
      success: true, 
      data: [],
      warning: 'Errore nel recupero degli stati delle estensioni.',
      configured: false
    } as ApiSuccessResponse);
  }
});
...
router.get('/extensions/:id/status', rbacMiddleware, requirePermission('view_telephony'), async (req, res) => {
  try {
    ...
    if (!externalId) {
      return res.status(400).json({ error: 'Extension not synced with EDGVoIP' });
    }
    ...
    return res.status(500).json({ error: 'Failed to get extension status' });
  }
});
...
router.get('/my-extension', rbacMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to retrieve extension credentials' });
  }
});
...
router.get('/extensions/:id/store-access', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch store access' });
  }
});
...
router.delete('/extension-store-access/:id', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    if (!deleted) {
      return res.status(404).json({ error: 'Store access not found' });
    }
...
router.get('/policies', rbacMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch contact policies' });
  }
});
...
router.post('/policies', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to create contact policy' });
  }
});
...
router.patch('/policies/:id', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to update contact policy' });
  }
});
...
router.delete('/policies/:id', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to delete contact policy' });
  }
});
...
router.get('/activity', rbacMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch VoIP activity log' });
  }
});
...
router.get('/cdrs', rbacMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch VoIP CDRs' });
  }
});
...
router.post('/cdr', async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to receive VoIP CDR' });
  }
});
...
router.post('/cdr/client', rbacMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to create VoIP CDR' });
  }
});
...
router.post('/provision/sync', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to sync provisioning' });
  }
});
...
router.get('/ai-config/:storeId', rbacMiddleware, async (req, res) => {
  try {
    ...
    const aiConfigs = trunks.map(trunk => ({
      ...
      aiFailoverExtension: trunk.aiFailoverExtension,
      aiTimePolicy: trunk.aiTimePolicy,
      host: trunk.host,
      isCurrentlyActive: trunk.aiAgentEnabled && isWithinBusinessHours(
        trunk.aiTimePolicy, 
        (trunk.aiTimePolicy as Record<string, unknown> | null)?.timezone as string || 'Europe/Rome'
      )
    }));
    ...
    return res.status(500).json({ error: 'Failed to fetch AI voice configuration' });
  }
});
...
router.post('/ai-config/:storeId', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    if (!trunk) {
      return res.status(404).json({ error: 'Trunk not found for this store' });
    }
    ...
    const [updated] = await db.update(voipTrunks)
      .set({
        ...
        aiFailoverExtension: validated.fallbackExtension,
        aiTimePolicy: validated.timeConditions,
        updatedAt: new Date()
      })
    ...
    return res.status(500).json({ error: 'Failed to update AI voice configuration' });
  }
});
...
router.post('/ai-sessions', async (req, res) => {
  try {
    ...
    logger.info('AI voice session saved', { 
      ...
      duration: newSession.durationSeconds,
      tenantId 
    });
    ...
    return res.status(500).json({ error: 'Failed to save AI session' });
  }
});
...
router.patch('/trunks/:id/ai-config', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to update trunk AI configuration' });
  }
});
...
router.get('/settings', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch VoIP settings' });
  }
});
...
router.put('/settings', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      // Create new config
      if (!validated.apiKey) {
        return res.status(400).json({ error: 'API key required for initial setup' });
      }
    ...
    return res.status(500).json({ error: 'Failed to update VoIP settings' });
  }
});
...
router.post('/settings/test', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    if (!config) {
      return res.status(404).json({ error: 'VoIP not configured for this tenant' });
    }
    ...
    return res.status(500).json({ error: 'Failed to test VoIP connection' });
  }
});
...
router.post('/settings/test-all', rbacMiddleware, requirePermission('manage_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to run API tests' });
  }
});
...
router.get('/logs', rbacMiddleware, requirePermission('view_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch VoIP logs' });
  }
});
...
router.get('/logs/stats', rbacMiddleware, requirePermission('view_telephony'), async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch VoIP log stats' });
  }
});
