...
import { mcpServers, users, tenants, userAssignments, roles } from '../db/schema/w3suite';
...
router.get('/google/start/:serverId', async (req: Request, res: Response) => {
  try {
    ...
    if (isAdminAssignment) {
      // Fetch all roles for current user (scope-based RBAC: scopeType='tenant', scopeId=tenantId)
      const userRoles = await db
        ...
        .where(and(
          ...
          eq(userAssignments.scopeId, tenantId)
        ));
...
router.get('/google/test/:serverId', async (req: Request, res: Response) => {
  try {
    ...
    const credentials = await decryptMCPCredentials(
      creds.encryptedCredentials as Record<string, any>,
      tenantId
    );

    // Get valid access token (auto-refresh if needed)
    // Note: GoogleOAuthService.getValidAccessToken requires userId for multi-user OAuth
    const userId = creds.userId || creds.createdBy || '';
    ...
    res.send(`
      ...
      <body>
        <div class="success">
          ...
          <p>Successfully connected ${result.connectedAccounts?.length || 0} account(s)</p>
...
router.get('/meta/test/:serverId', async (req: Request, res: Response) => {
  try {
    ...
    // Get valid access token (auto-refresh if needed)
    // Note: MetaOAuthService.getValidAccessToken requires userId for multi-user OAuth
    const userId = creds.userId || creds.createdBy || '';
