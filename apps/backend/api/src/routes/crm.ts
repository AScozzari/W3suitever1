...
// Enum type definitions for type-safe comparisons
type CrmLeadStatus = (typeof crmLeadStatusEnum.enumValues)[number];
type CrmDealStatus = (typeof crmDealStatusEnum.enumValues)[number];
type CrmCampaignStatus = (typeof crmCampaignStatusEnum.enumValues)[number];
type CrmCustomerType = (typeof crmCustomerTypeEnum.enumValues)[number];
type IdentityMatchStatus = (typeof identityMatchStatusEnum.enumValues)[number];
...
router.get('/leads', async (req, res) => {
  try {
    ...
    if (status) {
      conditions.push(eq(crmLeads.status, status as CrmLeadStatus));
    }
    if (storeId) {
  ...
  try {
    ...
    if (consents.length === 0) {
      ...
      if (leadConsents.length > 0) {
        ...
        // Map legacy boolean fields to new consent format (use synthetic IDs for inherited consents)
        consents = [
          {
            id: `${lead.leadId}-privacy_policy`,
            consentType: 'privacy_policy' as const,
            status: lead.privacyPolicyAccepted ? 'granted' : 'denied',
            grantedAt: lead.privacyPolicyAccepted ? (lead.consentTimestamp || lead.createdAt) : null,
            withdrawnAt: null,
            expiresAt: null,
      ...
      (async () => {
        try {
          ...
          const [campaign] = await db
            ...
            .from(crmCampaigns)
            .where(and(
              eq(crmCampaigns.id, validation.data.campaignId as string),
              eq(crmCampaigns.tenantId, tenantId)
            ))
            .limit(1);
          ...
          if (campaign) {
            ...
            if (campaign.routingMode === 'manual' && campaign.manualPipelineId1) {
              ...
              // Crea deal automaticamente nella pipeline (lead-pipeline relationship is through deal)
              const [deal] = await db
                .insert(crmDeals)
                .values({
                  tenantId,
                  storeId: lead.storeId,
                  ownerUserId: lead.ownerUserId || 'system',
                  ...
                  probability: 10,
                  status: 'open',
                  dealCreationSource: 'converted_from_lead'
                } as any)
                .returning();
...
router.get('/campaigns', async (req, res) => {
  try {
    ...
    // Build combined predicate with tenant isolation
    const conditions: any[] = [eq(crmCampaigns.tenantId, tenantId)];
    if (status) {
      conditions.push(eq(crmCampaigns.status, status as CrmCampaignStatus));
    }
    if (type) {
      conditions.push(eq(crmCampaigns.type as any, type as any));
    }
    ...
    const enrichedCampaigns = await Promise.all(campaigns.map(async (campaign: typeof crmCampaigns.$inferSelect) => {
      // Get lead counts
      ...
      // Get marketing channel names (campaign.marketingChannels is array of channel identifiers/codes)
      let marketingChannelNames = null;
      if (campaign.marketingChannels && campaign.marketingChannels.length > 0) {
        // marketingChannels is a text array of channel codes, not IDs
        marketingChannelNames = campaign.marketingChannels.sort().join(', ') || null;
      }
...
router.post('/campaigns', async (req, res) => {
  try {
    ...
    // Log if store has tracking config (but don't copy to campaign - tracking is resolved at runtime via store relation)
    if (campaignData.storeId) {
      ...
      if (storeTracking && storeTracking.gtmConfigured) {
        logger.info('Store has tracking config - will be used at runtime', {
...
router.get('/pipelines', async (req, res) => {
  try {
    ...
    // Build combined predicate with tenant isolation
    const conditions: any[] = [eq(crmPipelines.tenantId, tenantId)];
    if (domain) {
      conditions.push(eq(crmPipelines.domain as any, domain as any));
    }
    if (isActive !== undefined) {
...
router.get('/deals', async (req, res) => {
  try {
    ...
    if (status) {
      conditions.push(eq(crmDeals.status, status as CrmDealStatus));
    }
    if (pipelineId) {
  ...
  try {
    ...
    (async () => {
      try {
        ...
        if (validation.data.stage && validation.data.stage !== existingDeal.stage) {
          await dealNotificationService.notifyStateChange({
            dealId: id,
            tenantId,
            pipelineId: updated.pipelineId,
            fromStage: existingDeal.stage || 'unknown',
            toStage: validation.data.stage,
            dealTitle: `Deal #${updated.id.slice(0, 8)}`,
        ...
        if (validation.data.status === 'won' && existingDeal.status !== 'won') {
          await dealNotificationService.notifyDealWon({
            ...
            pipelineId: updated.pipelineId,
            dealTitle: `Deal #${updated.id.slice(0, 8)}`,
            value: updated.estimatedValue || undefined,
            ownerUserId: updated.ownerUserId || undefined,
            assignedTeamId: undefined
          });
        }
...
router.post('/deals/:id/duplicate', rbacMiddleware, requirePermission('crm.manage_deals'), async (req, res) => {
  try {
    ...
    (async () => {
      try {
        if (deal.pipelineId) {
          // Invalidate ROI analytics for the tenant
          await analyticsCacheService.invalidateROI(tenantId);
        }
      } catch (cacheError) {
        logger.error('Failed to invalidate analytics cache', {
          error: cacheError instanceof Error ? cacheError.message : 'Unknown error',
...
router.patch('/deals/:id/assign', async (req, res) => {
  try {
    ...
    // Note: tenantId is verified through the deal query above which already filters by tenantId
    const [pipelineSettings] = await db
      .select()
      .from(crmPipelineSettings)
      .where(eq(crmPipelineSettings.pipelineId, deal.pipelineId));
...
router.get('/customers', async (req, res) => {
  try {
    ...
    if (customerType) {
      conditions.push(eq(crmCustomers.customerType, customerType as CrmCustomerType));
    }
    if (status) {
...
router.get('/customers/:id/360', async (req, res) => {
  try {
    ...
    const totalRevenue = orders.reduce((sum: number, order: { totalAmount?: number | null }) => sum + Number(order.totalAmount || 0), 0);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        analytics: {
          ...
          wonDeals: deals.filter((d: { status?: string | null }) => d.status === 'won').length,
          lostDeals: deals.filter((d: { status?: string | null }) => d.status === 'lost').length,
...
router.patch('/leads/:id/status', async (req, res) => {
  try {
    ...
    const validStatuses = ['new', 'contacted', 'in_progress', 'qualified', 'converted', 'disqualified'];
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.update(crmLeads)
        .set({
          status: newStatus as CrmLeadStatus,
          updatedAt: new Date()
        })
      ...
      await tx.insert(leadStatusHistory).values({
        tenantId,
        leadId: req.params.id,
        oldStatusName: currentLead.currentStatus,
    ...
    logger.info('Lead status changed successfully', {
      leadId: req.params.id,
      oldStatus: currentLead.currentStatus,
      newStatus,
      tenantId,
      changedBy: req.user?.id
    });
    ...
    res.status(200).json({
      ...
      data: {
        leadId: req.params.id,
        oldStatus: {
          name: currentLead.currentStatus
        },
        newStatus: {
          name: newStatus
        }
  ...
  try {
    ...
    const mappedHistory = history.map(h => ({
      ...
      changedByName: h.changedByFirstName && h.changedByLastName 
        ...
        : h.changedByFirstName || h.changedByLastName || 'Unknown'
    }));
...
router.get('/person/:personId/omnichannel-timeline', rbacMiddleware, async (req, res) => {
  try {
    ...
    if (channel) {
      conditions.push(eq(crmOmnichannelInteractions.channel as any, channel as any));
    }
    if (startDate) {
      conditions.push(gte(crmOmnichannelInteractions.occurredAt, new Date(startDate as string)));
    }
    if (endDate) {
      conditions.push(lte(crmOmnichannelInteractions.occurredAt, new Date(endDate as string)));
    }
    ...
    const matches = await db
      ...
      .where(
        and(
          ...
          eq(crmIdentityMatches.status as any, status as any),
...
router.get('/identity-conflicts', rbacMiddleware, requirePermission('crm.manage_identities'), async (req, res) => {
  try {
    ...
    if (priority) {
      conditions.push(eq(crmIdentityConflicts.priority, priority as string));
    }
