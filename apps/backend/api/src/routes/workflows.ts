...
router.post('/requests', rbacMiddleware, async (req, res) => {
  try {
    ...
    // 🎯 DEPARTMENTAL CONSTRAINT: Workflow instance will be assigned through RequestTriggerService automation
    // Note: workflowInstanceId is not part of the create request schema

    // Create universal request - map validated schema to database columns
    const [newRequest] = await db
      .insert(universalRequests)
      .values({
        tenantId: requestData.tenantId,
        requesterId: requestData.requesterId,
        legalEntityId: requestData.legalEntityId || null,
        storeId: requestData.storeId || null,
        onBehalfOf: requestData.onBehalfOf || null,
        ...
        attachments: requestData.attachmentUrls || [],
    ...
    try {
      automationResult = await RequestTriggerService.triggerWorkflowForRequest(newRequest, tenantId as string);
...
router.post('/ai-generate', rbacMiddleware, requirePermission('workflow.create'), async (req, res) => {
  try {
    ...
    const usersContext = usersData.length > 0
      ? `\n\nUTENTI DISPONIBILI (usa questi ID reali):\n${usersData.slice(0, 10).map(u => `- ${u.firstName || ''} ${u.lastName || ''} (ID: "${u.id}", Email: ${u.email}, Department: ${u.department || 'N/A'})`).join('\n')}`
      : '';
...
${taskReminder.routing?.mode === 'manual' ? `\nPer MANUAL routing, seleziona gli ID appropriati dei team/utenti dalla lista sopra in base al department richiesto.` : ''}`;
    ...
    const existingFeatures = (tenantAISettings.featuresEnabled as Record<string, boolean>) || {};
...
router.get('/analytics', rbacMiddleware, requirePermission('workflow.read_analytics'), async (req, res) => {
  try {
    ...
    const peakHourRow = hourlyDistribution.rows.length > 0
      ...
      : null;
    const peakHour = peakHourRow?.hour != null ? parseInt(String(peakHourRow.hour), 10) : null;
    ...
    try {
      ...
      for (let i = 0; i < parsedWorkflow.totalSteps && i < 100; i++) { // Max 100 steps safety limit
        ...
        executionResults.push({
          nodeId: step.nodeId,
          nodeName: step.config?.name || step.nodeId,
          nodeType: step.type,
