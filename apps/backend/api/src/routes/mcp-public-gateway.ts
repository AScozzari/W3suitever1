...
router.post('/sse', mcpApiKeyAuth, async (req: McpAuthenticatedRequest, res: Response) => {
  ...
  try {
    // Use shared executeMcpTool for all methods (ensures parity with HTTP Streamable)
    const result = await executeMcpTool(tenantId, apiKeyId, authMethod, scopes, method, id, params, req);
...
// SSE POST handler now uses shared executeMcpTool() for parity with HTTP Streamable transport.
// The existing GET /sse handler at line ~1390 remains unchanged for backward compatibility.
